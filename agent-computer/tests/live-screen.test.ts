...
const asked = process.env.darbot_LIVE_SCREEN === "1";
...
function api(path: string, botId: string, init?: RequestInit) {
  return fetch(`${BASE}${path}`, {
    ...
    headers: {
      ...
      "x-darbot-bot-id": botId,
      "x-darbot-computer-token": TOKEN,
      ...(init?.headers ?? {}),
    },
  });
}
...
describe.skipIf(!asked)("printable punctuation from the live screen", () => {
  test("inserts a period using the browser key code sent by the surface", async () => {
    ...
    // Older darbot surfaces did not send the browser keyCode. Keep their punctuation usable while
