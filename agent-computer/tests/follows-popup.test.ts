 ...
 *   darbot_COMPUTER_BROWSER=1 bun test tests/follows-popup.test.ts
 */
const asked = process.env.darbot_COMPUTER_BROWSER === "1";
...
describe.skipIf(!asked)("the page a Bot is on", () => {
  test(
    ...
    async () => {
      ...
      const root = await mkdtemp(join(tmpdir(), "darbot-profiles-"));
