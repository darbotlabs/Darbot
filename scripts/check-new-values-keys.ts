...
async function lastReleaseTag(): Promise<string> {
  ...
  for (const tag of tags
    ...
    const has = Bun.spawnSync(
      ["git", "cat-file", "-e", `${tag}:charts/darbot/values.yaml`],
...
const before = new Set(
  paths(
    parse(await run(["git", "show", `${since}:charts/darbot/values.yaml`])),
  ),
);
const now = paths(parse(await Bun.file("charts/darbot/values.yaml").text()));
/*
 ...
 */
const rawChartValues = await Bun.file("charts/darbot/values.yaml").text();
...
function enableFor(component: string): string[] {
  ...
  return [
    ...
    "--set",
    `externalSecrets.data[${next}].remoteRef.key=darbot/worker-shared-secret`,
  ];
}
