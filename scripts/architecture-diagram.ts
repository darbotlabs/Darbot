/**
 * Draws the darbot architecture as a hand-drawn SVG, one file per colour scheme.
 *
...
function render(t: Theme): string {
  ...
  // ---- title -------------------------------------------------------------
  text(64, 56, "darbot", { size: 26, weight: 700, fill: t.ink });
  text(
    64,
    82,
  ...
  text(svX + 22, svY + 58, "darbotlm runtime, auth, roles, coworkers", {
    size: 13,
    fill: t.muted,
  });
  ...
  text(intX + 18, dbY + 32, "darbotlm Intelligence", {
    size: 14.5,
    weight: 650,
    fill: t.ink,
  });
  ...
  return [
    ...
    `<title>darbot architecture</title>`,
    `<desc>A turn goes from the app to the server, which sends it to a Bot over AG-UI. Every tool call ` +
      ...
      `and memory live in darbotlm Intelligence.</desc>`,
    ...out,
    `</svg>`,
  ].join("\n");
}
