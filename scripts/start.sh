#!/usr/bin/env bash
#
# Start the local darbot stack and verify each service answers as darbot.
...
ONE_COMPUTER_EACH="${darbot_ONE_COMPUTER_EACH:-true}"
export APP_PORT SERVER_PORT
SUPERVISOR_TOKEN="$(setting SUPERVISOR_TOKEN darbot-dev-supervisor-token)"
COMPUTER_TOKEN="$(setting COMPUTER_TOKEN darbot-dev-computer-token)"
...
WORKER_SHARED_SECRET="$(setting WORKER_SHARED_SECRET darbot-dev-worker-secret)"
...
# Does whatever holds this port answer as darbot, rather than merely answer?
#
...
identifies_as_darbot() {
  local port="$1" name="$2"
  case "$name" in
    # A field of this server's own payload. A stray 200 does not carry it.
    server)
      curl -fsS --max-time 3 "http://localhost:$port/api/darbotlm/info" 2>/dev/null \
...
require_free_or_ours() {
  ...
  who="$(holder "$port")"
  [ -z "$who" ] && return 0
  if identifies_as_darbot "$port" "$name"; then
    info "  $name: already up on $port ($who)"
    return 0
  fi
  red "  $name: port $port is held by something that is not darbot: $who"
  red "  Re-run with ${name^^}_PORT=<free port>, or stop that process yourself."
  exit 1
}

# As wait_for, but satisfied only by darbot answering, not by anything answering.
wait_for_darbot() {
  local port="$1" name="$2" tries="${3:-40}"
  for _ in $(seq 1 "$tries"); do
    identifies_as_darbot "$port" "$name" && { green "  $name ready"; return 0; }
    sleep 1
  done
  red "  $name never answered as darbot on port $port"
...
for table in agent_profiles agent_preferences; do
  if ! docker compose exec -T postgres \
       psql -U darbot -d darbot -tAc "select to_regclass('public.$table')" 2>/dev/null \
       | grep -q "^$table$"; then
...
#
# A server that answers as darbot can still be one no worker can hand a routine to. The worker
...
if identifies_as_darbot "$SERVER_PORT" server; then
...
wait_for_darbot "$SERVER_PORT" server
...
if ! pgrep -f "bun worker/src/index.ts" >/dev/null 2>&1; then
  WORKER_DATABASE_URL="$(setting DATABASE_URL postgres://darbot:darbot@localhost:5432/darbot)"
  (cd "$ROOT" && \
...
INFO="$(curl -fsS --max-time 8 "http://localhost:$SERVER_PORT/api/darbotlm/info")"
python3 - "$INFO" <<'PY'
import json, sys
...
if status != "valid":
    ...
    print("\033[31m  Check INTELLIGENCE_API_KEY: npx darbotlm@latest login && npx darbotlm@latest project select\033[0m")
...
if ! identifies_as_darbot "$APP_PORT" app; then
  (cd app && bun run dev --port "$APP_PORT" --strictPort >"$LOGS/app.log" 2>&1 &)
fi
wait_for_darbot "$APP_PORT" app
