...
The contract with the rest of darbot is the same one the other Bots meet, and it is small:
...
# The one header darbot's server sends when it calls a managed Bot. Same name the TypeScript Bots
...
def _model() -> str:
    """The provider and model darbot chose, in the form litellm wants.
...
def _darbot_prepare_crewai_inputs(
    *,
    state: dict,
...
crewai_endpoint.crewai_prepare_inputs = _darbot_prepare_crewai_inputs


class darbotFlow(Flow):
...
add_crewai_flow_fastapi_endpoint(app, darbotFlow(), "/")
