...
def boundary(monkeypatch):
        ...
        def do_POST(self):
            ...
            if self.path == "/api/agent-tools/call":
                captured["callback"].append(
                    {"body": body, "token": self.headers.get("x-darbot-agent-token")}
                )
                status = captured["callback_status"]
                if authorize := captured.get("callback_authorize"):
                    status = authorize(body, self.headers.get("x-darbot-agent-token"))
    ...
    monkeypatch.setenv("darbot_TOOL_URL", base + "/api/agent-tools/call")
...
def run_input(
    ...
    *,
    ...
    return {
        ...
        "state": {},
        "forwardedProps": {
            "darbotDeploymentTools": list(deployment),
            "darbotRun": assertion,
        },
    }
...
async def run_protocol(body, *, allow_error=False):
    async with httpx.AsyncClient(
        ...
        response = await client.post(
            "/", json=body, headers={"x-darbot-agent-token": "synthetic-server-token"}
        )
