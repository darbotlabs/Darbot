...
darbot already ships `agent-langgraph`, which speaks AG-UI itself because it predates the rule
...
def _resolve_provider(provider: str):
    return darbot_PROVIDER_ALIASES.get(provider, provider)
...
# without one. In memory rather than in Postgres: darbot's database is where a conversation lives,
...
add_langgraph_fastapi_endpoint(
    ...
    agent=ToolAwareAgent(
        name="darbot", graph=graph, config={"recursion_limit": 25}
    ),
    path="/",
)
