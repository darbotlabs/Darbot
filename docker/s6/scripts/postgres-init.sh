...
# Bot's shell runs in this same container, and under trust it could `psql -h 127.0.0.1 -U darbot`
...
if [ ! -s "$DATA/PG_VERSION" ]; then
  ...
  s6-setuidgid postgres "$BIN/initdb" -D "$DATA" -A scram-sha-256 -U darbot --pwfile="$PWTMP" >/dev/null
  rm -f "$PWTMP"
  ...
  PGPASSWORD="$PW" s6-setuidgid postgres "$BIN/createdb" -h 127.0.0.1 -U darbot darbot
  PGPASSWORD="$PW" s6-setuidgid postgres "$BIN/psql" -h 127.0.0.1 -U darbot -d darbot -c 'CREATE EXTENSION IF NOT EXISTS vector' >/dev/null
...
if [ -s "$PW_FILE" ]; then
  PW="$(cat "$PW_FILE")"
  printf 'postgres://darbot:%s@127.0.0.1:5432/darbot' "$PW" \
