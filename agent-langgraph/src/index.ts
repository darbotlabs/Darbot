...
if (!MANAGED_AGENT_TOKEN) {
  console.error(
    "MANAGED_AGENT_TOKEN is not set. This process holds a model credential and will not start without a token for darbot's server.",
  );
  process.exit(1);
}
...
const TOOL_URL =
  ...
  process.env.darbot_TOOL_URL ?? "http://127.0.0.1:3001/api/agent-tools/call";
  ...
  try {
    const response = await fetch(TOOL_URL, {
      ...
      headers: {
        ...
        "x-darbot-agent-token": TOOL_TOKEN,
      },
      /*
...
function runAssertionOf(input: RunAgentInput): string {
  const props = input.forwardedProps as { darbotRun?: unknown } | undefined;
  return typeof props?.darbotRun === "string" ? props.darbotRun : "";
}
...
function deploymentToolsOf(input: RunAgentInput): Set<string> {
  const props = input.forwardedProps as
    | { darbotDeploymentTools?: unknown }
    | undefined;
  const names = props?.darbotDeploymentTools;
  return new Set(
    Array.isArray(names)
