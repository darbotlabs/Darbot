# darbot on Kubernetes

Runs darbot on any Kubernetes cluster: EKS, GKE, AKS, or your own. One chart, five targets, and the
...
**An image the cluster can pull.** A release publishes `ghcr.io/darbotlm/darbot:vX.Y.Z`
...
```sh
docker manifest inspect ghcr.io/darbotlm/darbot:v0.0.9 | grep architecture
```

**Intelligence credentials.** darbot requires darbotlm Intelligence and the chart refuses to
install without `secrets.intelligenceApiKey`. It comes from the CLI, on any machine with a browser:

```sh
npx --yes darbotlm@latest login           # browser sign-in
npx --yes darbotlm@latest project select  # prints the cpk-... runtime key
```
...
licence. `npx --yes darbotlm@latest license --print` prints one without writing it to a local
...
```sh
kubectl create namespace darbot
kubectl -n darbot create secret generic darbot-database \
  --from-literal=database-url='postgresql://USER:PASSWORD@HOST:5432/darbot?sslmode=require'
```

Then `--set database.existingSecret=darbot-database`. The key must be `database-url`, or name a
...
kind: ClusterConfig
metadata: { name: darbot, region: us-east-2, version: "1.34" }
iam: { withOIDC: true }
addons:
  - name: vpc-cni
  - name: coredns
...
```sh
helm dependency build charts/darbot
helm upgrade --install darbot charts/darbot \
  --namespace darbot --create-namespace \
...
--set database.existingSecret=darbot-database   # key: database-url
```
...
somebody can try darbot in one command.
...
- **Pre-create it AS the migrating role.** Where that grant is not on offer, the administrative role
  ...
  ```sql
  SET ROLE darbot_migrator;      -- the role in DATABASE_URL
...
```sh
helm template darbot charts/darbot -f charts/darbot/ci/eks-values.yaml
```
...
darbot is a shell for somebody else's agent, and `config.managedAgent.url` is where that agent goes:
