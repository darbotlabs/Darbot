...
{{- define "darbot.labels" -}}
helm.sh/chart: {{ include "darbot.chart" . }}
{{ include "darbot.selectorLabels" . }}
{{- if .Chart.AppVersion }}
...
{{- define "darbot.selectorLabels" -}}
app.kubernetes.io/name: {{ include "darbot.name" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
{{- end -}}

{{/* Labels for one component, so two workloads in one release never select each other's pods. */}}
{{- define "darbot.componentLabels" -}}
{{ include "darbot.labels" .root }}
app.kubernetes.io/component: {{ .component }}
{{- end -}}

{{- define "darbot.componentSelectorLabels" -}}
{{ include "darbot.selectorLabels" .root }}
app.kubernetes.io/component: {{ .component }}
{{- end -}}

{{- define "darbot.componentName" -}}
{{- printf "%s-%s" (include "darbot.fullname" .root) .component | trunc 63 | trimSuffix "-" -}}
{{- end -}}
...
*/}}
{{- define "darbot.cronJobName" -}}
{{- $room := int (max 1 (sub 51 (len .component))) -}}
{{- $prefix := include "darbot.fullname" .root | trunc $room | trimSuffix "-" -}}
...
*/}}
{{- define "darbot.attachmentsCullerEnabled" -}}
...
*/}}
{{- define "darbot.kubernetesApiCidr" -}}
...
{{- define "darbot.serviceAccountName" -}}
{{- if .Values.serviceAccount.create -}}
{{- default (include "darbot.fullname" .) .Values.serviceAccount.name -}}
{{- else -}}
...
{{- define "darbot.secretName" -}}
{{- default (printf "%s-secrets" (include "darbot.fullname" .)) .Values.secrets.existingSecret -}}
{{- end -}}

{{- define "darbot.configMapName" -}}
{{- printf "%s-config" (include "darbot.fullname" .) -}}
{{- end -}}
...
- name: AGENT_COMPUTER_URL
  value: http://{{ include "darbot.componentName" (dict "root" . "component" "computer") }}-0.{{ include "darbot.componentName" (dict "root" . "component" "computer") }}:4100
...
- name: COMPUTER_SANDBOX_TEMPLATE_FILE
  value: /etc/darbot/sandbox-template.json
{{- end }}
{{- /*
...
- name: INTELLIGENCE_API_KEY
  valueFrom:
    secretKeyRef:
      name: {{ include "darbot.secretName" . }}
      key: intelligence-api-key
{{- if .Values.secrets.licenseToken }}
- name: darbotlm_LICENSE_TOKEN
  valueFrom:
    secretKeyRef:
      name: {{ include "darbot.secretName" . }}
      key: license-token
...
- name: COMPUTER_TOKEN
  valueFrom:
    secretKeyRef:
      name: {{ default (include "darbot.secretName" .) .Values.computers.existingTokenSecret }}
      key: computer-token
      optional: {{ eq .Values.computers.mode "external" }}
{{- /*
  One definition, for the same reason `darbot.databaseUrlEnv` is one (see its comment above): the
...
podAntiAffinity:
  requiredDuringSchedulingIgnoredDuringExecution:
    - topologyKey: kubernetes.io/hostname
      labelSelector:
        matchLabels:
{{ include "darbot.componentSelectorLabels" (dict "root" $root "component" $component) | indent 10 }}
  ...
  preferredDuringSchedulingIgnoredDuringExecution:
    - weight: 100
      podAffinityTerm:
        ...
        labelSelector:
          matchLabels:
{{ include "darbot.componentSelectorLabels" (dict "root" $root "component" $component) | indent 12 }}
{{- end -}}
{{- end -}}
...
*/}}
{{- define "darbot.sandboxPodTemplate" -}}
{{- $spec := dict
  "podTemplate" (dict
