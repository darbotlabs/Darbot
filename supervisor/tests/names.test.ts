...
describe("the ids a supervisor will accept", () => {
  ...
  test("ids that could name somebody else's container are refused", () => {
    for (const id of [
      "darbot-postgres", // starts plausibly, still only ever becomes <namespace>-computer-*
      "../postgres",
  ...
  test("the ownership labels are stable", () => {
    ...
    expect(OWNER_LABEL).toBe("darbot.supervisor");
    expect(BOT_LABEL).toBe("darbot.bot-id");
    expect(NAMESPACE_LABEL).toBe("darbot.namespace");
  });
  ...
  test("the default namespace is the one existing deployments already use", () => {
    ...
    expect(DEFAULT_NAMESPACE).toBe("darbot");
  });
});
