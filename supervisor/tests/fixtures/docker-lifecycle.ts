...
const IMAGE = `darbot-supervisor-fixture:${namespace}-one`;
const OTHER = `darbot-supervisor-fixture:${namespace}-two`;
...
async function remove(container: string) {
  ...
  try {
    ...
    if (
      labels?.["darbot.test-fixture"] !== namespace &&
      !(
        labels?.["darbot.namespace"] === namespace &&
        labels?.["darbot.bot-id"] === BOT
      )
    ) {
...
async function removeVolumes() {
  for (const volume of [names.profileVolume, names.workspaceVolume]) {
    try {
      ...
      if (
        info.Labels?.["darbot.namespace"] !== namespace ||
        info.Labels?.["darbot.bot-id"] !== BOT
      ) {
...
afterAll(async () => {
  ...
  for (const image of fixtureImages) {
    ...
    if (info.Config.Labels?.["darbot.test-fixture"] !== namespace)
...
async function plant(labels: Record<string, string>, health?: boolean) {
  ...
  await withDocker().docker.createContainer({
    ...
    Image: IMAGE,
    Labels: { ...labels, "darbot.test-fixture": namespace },
    ...(health
      ? {}
      : {
          Cmd: ["sleep", "600"],
...
const OURS = {
  "darbot.supervisor": "true",
  "darbot.namespace": namespace,
  "darbot.bot-id": BOT,
};
