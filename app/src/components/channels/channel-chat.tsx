...
export function ChannelChat({
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        await joinWithin({
          connect: darbotlm.connectAgent({ agent }),
  ...
  }, [darbotlm, agent, isReady, channel.threadId, runtimeAgentId]);
   ...
   *
   * `darbotlm.runAgent` DOES NOT REJECT ON A FAILED RUN. `darbotlmCore.runAgent` catches
   ...
   * handle that `darbotlm.runAgent` does not hand back.
   */
   ...
   * is over.
   *
   * `runsInFlight` counts what Stop can actually reach: the run `darbotlm.runAgent` opens, and
   ...
   * idle in every one of those gaps — the truth about the wire and a lie about the turn. darbot
  ...
  const deliver = async (
    ...
    try {
      await darbotlm.runAgent({ agent: target });
    } finally {
      setRunsInFlight((count) => count - 1);
    }
