 ...
 * the port each announces on `window.__darbot_WS_PORT__`. Nothing announces it in the built bundle
 * the server hands out, so a production page has no port to override with and stays same-origin.
 */
declare global {
  interface Window {
    __darbot_WS_PORT__?: string;
  }
}

function announcedPort(): string {
  return typeof window !== "undefined" &&
    typeof window.__darbot_WS_PORT__ === "string"
    ? window.__darbot_WS_PORT__
    : "";
}
