...
struct Shell {
    ...
    last_failure: Mutex<Option<darbot_desktop_lib::problem::Problem>>,
    ...
    signing_in_to_intelligence:
        Mutex<Option<darbot_desktop_lib::intelligence::SigningInToIntelligence>>,
    ...
    signing_in_to_chatgpt: Mutex<Option<darbot_desktop_lib::plan::SigningInToChatGpt>>,
    ...
    signing_in: Mutex<Option<darbot_desktop_lib::plan::SigningIn>>,
...
impl<'a> StartAttempt<'a> {
    fn begin(shell: &'a Shell) -> Result<Self, Problem> {
        ...
        shell.starting.compare_exchange(false, true, SeqCst, SeqCst).map_err(|_| {
            Problem::plain("darbot is already starting or finishing a cancelled startup. Wait for it to finish, then try again.")
        })?;
        Ok(Self {
            shell,
    ...
    fn cancelled() -> Problem {
        Problem::plain("darbot startup was cancelled by Stop. Start again when you are ready.")
    }
}
...
struct SavedConfiguration {
    ...
    model: Option<darbot_desktop_lib::saved_intent::ModelIntent>,
}
...
fn ready_responding_engine_after_compose_repair(
    ...
    let Some(address) = ready.address.clone().filter(|_| ready.responding) else {
        return Err(Problem::with(
            "darbot installed Compose, but the container engine is not answering. Try again.",
            ready.detail,
        ));
    };
    if !composes(&address) {
...
async fn engine_ready(app: &tauri::AppHandle) -> Result<engine::Address, Problem> {
    ...
    .map_err(|error| {
        Problem::with(
            "darbot could not check the software it runs on. Try again.",
    ...
    report(
        ...
        "install-engine",
        true,
        "Looking for the software darbot runs on.",
    );
    ...
    .map_err(|error| {
        Problem::with(
            "darbot could not install the software it needs. Try again.",
    ...
    ready
        ...
        .ok_or_else(|| {
            Problem::with(
                "darbot set up the software it runs on, but it is still not answering. Try again.",
                ready.detail,
            )
        })
}
...
async fn deployment_ready<R: tauri::Runtime>(
    ...
    .map_err(|error| {
        ...
        Problem::with(
            "darbot could not download what it needs to run. Check the internet \
...
fn sign_in_reference(
    ...
    reference(root, published).map_err(|error| {
        Problem::with(
            "This version of darbot cannot sign in to that plan. Use an API key instead, or \
             update darbot.",
            error,
        )
    })
...
impl ChosenModel {
    fn into_credential(self, root: &Path) -> Result<darbot_env::ModelCredential, Problem> {
    ...
    fn into_credential_with(
    ...
    ) -> Result<darbot_env::ModelCredential, Problem> {
        ...
        match (self.provider.as_str(), self.login.as_str()) {
            ("openai", "api-key") => {
                ...
                Ok(darbot_env::ModelCredential::OpenAi { api_key })
            }
            ("anthropic", "api-key") => {
                let api_key = if saved {
                ...
                Ok(darbot_env::ModelCredential::Anthropic { api_key })
            }
            ("anthropic", "plan") => {
                let token = if saved {
                ...
                Ok(darbot_env::ModelCredential::ClaudePlan { token })
            }
            /*
            ...
            ("openai", "plan") => {
                let store = if saved {
                    darbot_env::read_plan_store(root)
                        .map_err(|error| {
                            Problem::with(
                                "darbot could not read the saved ChatGPT sign-in.",
                                format!("{}: {error}", root.join(darbot_env::CHATGPT_STORE_FILE).display()),
                            )
                        })?
                        .unwrap_or_default()
                } else {
                ...
                Ok(darbot_env::ModelCredential::ChatGptPlan { store })
            }
            ("openai-compatible", "endpoint") => {
                ...
                let api_key = if saved {
                    use darbot_desktop_lib::saved_intent::{
                ...
                Ok(darbot_env::ModelCredential::Compatible {
                    base_url,
...
fn saved_secret(root: &Path, key: &str) -> Result<String, Problem> {
    darbot_desktop_lib::vault::already_given_no_ui(root, &root.join(".env"), &[key])
...
fn intelligence_key_for_start(
    ...
    if key.trim().is_empty() {
        return Err("That saved darbotlm connection is no longer available. Sign in again or enter a project key.".into());
    }
    Ok(key)
}
...
fn require_existing_encryption_key(
    ...
    let configured = darbot_desktop_lib::saved_intent::SavedIntent::read(root)
        .model
        .is_some()
        || darbot_env::already_set(&root.join(".env"), &["DATABASE_URL"])
            .contains_key("DATABASE_URL");
    if configured
        && !secrets
            .get("KEY_ENCRYPTION_KEY")
            .is_some_and(|value| darbot_env::usable_encryption_key(value))
    {
        return Err(Problem::plain(
            "This installation's saved encryption key is missing, invalid, or public. Restore its original private key from backup, or get help preserving its saved data. darbot will not replace the key automatically.",
        ));
    }
    Ok(())
}
...
) -> Result<(), darbot_desktop_lib::problem::Problem> {
...
async fn start_stack_inner<R: tauri::Runtime>(
    ...
    let attempt = StartAttempt::begin(&shell)?;
    {
        ...
        {
            return Err(Problem::plain(
                "darbot still has services from another installation to stop. Choose Stop darbot before starting in a different folder.",
            ));
        }
    ...
    .map_err(|error| match error {
        ...
        // about a pick. "darbot v0.0.8 does not include agent-langgraph-agui" is the
        ...
        harness::PickedAfterDeploymentError::Harness(error) => Problem::with(
            "This version of darbot does not include the Bot you picked. Go back and choose \
                 another, or update darbot.",
            error,
        ),
    })?;
    ...
    let (logs, bun, mut secrets) = {
        ...
        let existing_secrets = darbot_desktop_lib::vault::already_given_no_ui(
            &root,
            &root.join(".env"),
            &darbot_env::MINTED[..],
        )?;
        ...
        let settings = darbot_env::compose(
            &darbot_env::Intelligence {
                api_url,
                gateway_ws_url,
                api_key,
            },
         ...
         */
        let (settings, mut secrets) = darbot_desktop_lib::vault::split(settings);
        /*
        ...
        darbot_desktop_lib::saved_intent::persist_configuration(
            &root,
            &settings,
            &secrets,
            &purge,
        ...
        // Said before rather than after. On a machine that has never run darbot this pulls five
        ...
        *shell.containers.lock().unwrap() = Some(ContainerDeployment {
        ...
        let ports = darbot_env::Ports::default();
        if reclaimed > 0 {
    ...
    secrets.insert("darbot_DESKTOP_HOST_TOKEN".into(), host_token.clone());
    ...
    let generation = start_host_processes(
        ...
        move |started| {
            stack::wait_until_answering(
                ...
                &logs_for_wait,
                &stack::Ready {
                    api: darbot_env::Ports::default().server,
                    app: darbot_env::Ports::default().app,
                },
    ...
    let config = host_access::HostAccessConfig::new(
        format!("http://127.0.0.1:{}", darbot_env::Ports::default().server),
        host_token,
        address,
...
where
    C: FnOnce(&Path) -> Result<usize, darbot_desktop_lib::problem::Problem>,
...
fn down_owned_containers(shell: &Shell, root: &Path) -> Result<(), String> {
    ...
    match containers.as_ref() {
        ...
        None if root.join("docker-compose.yml").exists() => Err(
            "darbot has no runtime ownership for this installation. Stop its containers using the original engine and context before starting darbot again.".into(),
        ),
        None => Ok(()),
    }
}
...
fn stop_held_process_handles(
    ...
    for (name, child) in children.iter_mut() {
        let failure = |error| {
            Problem::with(
                "darbot could not stop one of its host processes.",
    ...
    C: FnOnce(&Path) -> Result<usize, Problem>,
{
    ...
    let result = match (host_access_result, result) {
        ...
        (Err(error), Ok(_)) => Err(Problem::with(
            "darbot could not stop a folder operation.",
            error,
        )),
        (Err(error), Err(problem)) => Err(Problem::with(
            "darbot could not finish stopping its work.",
...
fn require_no_exited_compose_services(
    ...
    Err(Problem::with(
        "Part of darbot stopped during startup.",
        detail,
    ))
}
...
fn cleanup_before_start<R, C>(
    ...
    root: &Path,
    cleanup: C,
) -> Result<usize, darbot_desktop_lib::problem::Problem>
where
    R: tauri::Runtime,
    C: FnOnce(&Path) -> Result<usize, darbot_desktop_lib::problem::Problem>,
{
...
fn problem_detail(problem: darbot_desktop_lib::problem::Problem) -> String {
...
fn quit_cleanup_failure_summary(failure: QuitCleanupFailure) -> &'static str {
    match failure {
        QuitCleanupFailure::HostProcesses => "darbot could not confirm all app processes stopped.",
        QuitCleanupFailure::Containers => "darbot could not confirm all containers stopped.",
    }
}
...
fn read_quit_cleanup_notice(root: &Path) -> Result<Option<Problem>, Problem> {
    ...
    let file = match std::fs::File::open(&path) {
        ...
        Err(error) => {
            return Err(Problem::with(
                "darbot could not read its previous shutdown notice.",
    ...
    std::fs::remove_file(&path).map_err(|error| {
        Problem::with(
            "darbot could not clear its previous shutdown notice.",
            format!("{}: {error}", path.display()),
        )
    })?;
    let detail = notice
    ...
    Ok(Some(Problem::with(
        "darbot had trouble shutting down last time.",
        detail,
    )))
}
...
/// Show darbot itself in this window.
///
...
/// double-clicked darbot to get darbot.
///
...
#[tauri::command]
fn show_darbot<R: tauri::Runtime>(app: tauri::AppHandle<R>) -> Result<(), String> {
    show_darbot_on(app, &darbot_env::Ports::default())
}

fn show_darbot_on<R: tauri::Runtime>(
    app: tauri::AppHandle<R>,
    ports: &darbot_env::Ports,
) -> Result<(), String> {
    let port = ports.app;
    ...
    if recovery_required_or_pending_quit_notice(&shell, &root) {
        return Err("Part of darbot needs recovery. Try starting darbot once more.".into());
    }
    let url = owned_app_url(&root, ports).ok_or_else(|| {
        format!("darbot could not verify its app on port {port} belongs to this installation. Try starting darbot again.")
    })?;
    eprintln!("[show] navigating the window to {url}");
    let window = app
        .get_webview_window("main")
        .ok_or("the darbot window is not there to show it in")?;
    let outcome = window
        .navigate(
            url.parse()
                .map_err(|error| format!("{url} is not a URL: {error}"))?,
        )
        .map_err(|error| format!("could not show darbot: {error}"));
...
fn configured_setup_url(
    ...
    let window = config
        ...
        .ok_or("the darbot setup window is not configured")?;
    match &window.url {
        ...
        _ => Err("the darbot setup window URL is not supported".into()),
    }
}
...
fn show_setup<R: tauri::Runtime>(app: tauri::AppHandle<R>) -> Result<(), String> {
    let window = app
        .get_webview_window("main")
        .ok_or("the darbot window is not there")?;
...
/// running, and the port check then reports darbot as a foreign process holding its own port.
///
...
fn already_running<R: tauri::Runtime>(app: tauri::AppHandle<R>, root: String) -> bool {
    ...
    !recovery_required_or_pending_quit_notice(&shell, &root)
        && already_running_at(&root, &darbot_env::Ports::default())
}

fn already_running_at(root: &Path, ports: &darbot_env::Ports) -> bool {
    owned_app_url(root, ports).is_some()
}

/// Neither an owned API nor an answering app port alone authorizes showing a deployment.
fn owned_app_url(root: &Path, ports: &darbot_env::Ports) -> Option<String> {
...
fn last_failure<R: tauri::Runtime>(
    app: tauri::AppHandle<R>,
) -> Option<darbot_desktop_lib::problem::Problem> {
    let shell = app.state::<Shell>();
...
async fn ask_the_bot<R: tauri::Runtime>(
    ...
    root: String,
    question: String,
) -> Result<String, darbot_desktop_lib::problem::Problem> {
...
fn ask_saved_settings(root: &Path) -> Result<std::collections::BTreeMap<String, String>, Problem> {
    darbot_desktop_lib::vault::already_given_no_ui(
        root,
        &root.join(".env"),
        &[
            "PICKED_HARNESS_URL",
...
async fn ask_the_bot_with_settings(
    ...
    // The picked harness if there is one, and the Bot that ships with darbot if there is not.
    ...
    if endpoint.trim().is_empty() || token.trim().is_empty() {
        return Err(darbot_desktop_lib::problem::Problem::plain(
            "darbot cannot find the Bot it just set up. Stop darbot and start it again.",
        ));
    }

    let question = if question.trim().is_empty() {
        darbot_desktop_lib::ask::SUGGESTED.to_string()
    } else {
        question
    };

    let asked = tauri::async_runtime::spawn_blocking(move || {
        match darbot_desktop_lib::ask::ask_harness(
            &endpoint,
            &token,
            &question,
            kind.as_deref(),
    ...
    .await
    .map_err(|error| {
        darbot_desktop_lib::problem::Problem::plain(format!(
    ...
    match asked {
        ...
        Err(None) => {
            let log = log_service
                ...
                .unwrap_or_default();
            Err(darbot_desktop_lib::ask::why_nothing_came_back(&log))
        }
    }
}
...
fn already_configured(root: String) -> AlreadyConfigured {
    ...
    let mut values = darbot_desktop_lib::vault::already_given_file_only(
        &env_file,
        &[
    ...
    use darbot_desktop_lib::saved_intent::{Category, SavedIntent};
        ...
        saved: SavedConfiguration {
            ...
            model_sessions: SavedModelSessions {
                openai: hint(
                    ...
                    darbot_env::saved_chatgpt_plan_store(&root),
                ),
...
async fn begin_claude_sign_in(app: tauri::AppHandle, root: String) -> Result<String, Problem> {
    ...
    let image = sign_in_image(&app, &root, darbot_desktop_lib::plan::SIGN_IN_IMAGE).await?;
    let telemetry_app = app.clone();
    let (signing, url) = tauri::async_runtime::spawn_blocking(move || {
        ...
        darbot_desktop_lib::plan::SigningIn::begin(&address, &image).map_err(Problem::from)
    })
    .await
    .map_err(|error| {
        Problem::with(
...
async fn begin_chatgpt_sign_in(
    ...
    let (signing, url) = tauri::async_runtime::spawn_blocking(move || {
        ...
        darbot_desktop_lib::plan::SigningInToChatGpt::begin(&address, &image)
    })
    .await
    .map_err(|error| {
        Problem::with(
...
async fn begin_intelligence_sign_in(app: tauri::AppHandle) -> Result<String, String> {
    let (signing, url) = darbot_desktop_lib::intelligence::SigningInToIntelligence::begin()?;
    *app.state::<Shell>()
        .signing_in_to_intelligence
        .lock()
        .unwrap() = Some(signing);
...
) -> Result<Vec<darbot_desktop_lib::intelligence::Project>, darbot_desktop_lib::problem::Problem>
{
    let signing = app
        ...
        .take()
        .ok_or_else(|| {
            darbot_desktop_lib::problem::Problem::plain(
...
async fn intelligence_key_for(
    ...
    .await
    .map_err(|error| {
        darbot_desktop_lib::problem::Problem::plain(format!("A key could not be created: {error}"))
    })?
}
        ...
        loop {
            ...
            for name in dead {
                ...
                if !watch.should_restart(std::time::Instant::now()) {
                     ...
                     * darbot stopped and that starting again is the thing to try.
                     */
                    *shell.last_failure.lock().unwrap() =
                        Some(darbot_desktop_lib::problem::Problem::with(
                            format!(
                                "Part of darbot ({name}) stopped and could not be started again. \
                                 Try starting darbot once more."
                            ),
                            reason,
                        ));
                    // Back to the setup screen. By now the window is showing darbot, and darbot
    ...
    F: FnOnce() -> std::io::Result<std::process::Child>,
{
    ...
    let child = spawn().map_err(|error| {
        Problem::with(
            format!("darbot could not restart {name}."),
            error.to_string(),
        )
    })?;
    #[cfg(unix)]
...
/// Point the window at darbot if it is up, and at the setup screen if it is not.
///
...
fn show_whichever_applies(app: &tauri::AppHandle) {
    restore_window_on(app, &darbot_env::Ports::default());
}

fn restore_window_on<R: tauri::Runtime>(app: &tauri::AppHandle<R>, ports: &darbot_env::Ports) {
    ...
    F: FnOnce(T) + Send + 'static,
{
    std::thread::Builder::new()
        .name("darbot-second-instance-restore".into())
...
fn restore_after_second_instance(app: &tauri::AppHandle) {
    ...
    if let Err(error) = schedule_second_instance_restore(app, |app| {
        ...
        report(
            ...
            "open",
            false,
            format!("darbot could not show the existing window: {error}"),
        );
    }
}

fn publish_quit_notice_failure<R: tauri::Runtime>(app: tauri::AppHandle<R>, error: String) {
    let problem = Problem::with("darbot could not record a shutdown problem.", error);
    let shell = app.state::<Shell>();
    {
        let _startup = shell.startup.lock().unwrap();
        let root = cleanup_root(&shell, &stack::default_root());
    ...
    std::thread::spawn(move || {
        ...
        match stop_everything(&app, &root_path) {
            Ok(()) => {
                ...
                report(&app, "stopped", true, "darbot has been stopped");
            }
...
fn main() {
    tauri::Builder::default()
        ...
        // with Back, Refresh, Save as and Print: Back walks the window out of darbot with nothing
        ...
        // asked to refuse. Every navigation, because the window navigates to darbot and back.
        ...
        .setup(|app| {
            ...
            // Where the Compose provider darbot installs itself lives, told once so every engine
            ...
            let open = MenuItem::with_id(app, "open", "Open darbot", true, None::<&str>)?;
            let stop = MenuItem::with_id(app, "stop", "Stop darbot", true, None::<&str>)?;
            let quit = MenuItem::with_id(app, "quit", "Quit", true, quit_menu_accelerator())?;
            let menu = Menu::with_items(app, &[&open, &stop, &quit])?;

            TrayIconBuilder::with_id("darbot")
                .icon(tray::icon())
        ...
        .expect("the darbot window could not be created")
        .run(|app, event| {
            match event {
                #[cfg(target_os = "macos")]
                ...
                tauri::RunEvent::ExitRequested { api, code, .. } => {
                    ...
                    if let Err(error) = request_quit_with(
                        ...
                        report(
                            ...
                            "quit",
                            false,
                            "darbot could not start shutting down. Try Quit again.",
                        );
                    }
                }
                _ => {}
...
mod tests {
    ...
    fn quit_cleanup_notice_is_known_safe_bounded_and_consumed_once() {
        let root = temp_root("darbot-quit-cleanup-notice");
        let lines = vec![
            "[exit] cleanup failed: Compose down failed: /Users/alice/darbot/docker-compose.yml refused token=secret".to_string(),
            "[exit] cleanup failed: C:\\Users\\alice\\darbot\\owned.exe OAuth password".to_string(),
        ];
        ...
        assert_eq!(first.said, "darbot had trouble shutting down last time.");
    ...
    fn public_already_configured_reads_only_passive_files() {
        ...
        for metadata in [
            ...
            if let Some(metadata) = metadata {
                std::fs::write(root.join(darbot_desktop_lib::saved_intent::FILE), metadata)
                    .unwrap();
            }
    ...
    fn plan_sign_in_boundary_uses_selected_root_for_deploy_and_reference() {
        ...
        assert_eq!(
            ...
            Some((
                selected.clone(),
                darbot_desktop_lib::plan::CHATGPT_SIGN_IN_IMAGE.to_string()
            ))
        );
    ...
    fn command_roots_trim_paste_padding_and_preserve_interior_spaces() {
        let root = temp_root("darbot-command-root My Files");
        std::fs::create_dir_all(&root).unwrap();
    ...
    fn already_configured_trims_pasted_root_and_preserves_interior_spaces() {
        let root = temp_root("darbot-pasted-root My Files");
        std::fs::create_dir_all(&root).unwrap();
        std::fs::write(
            root.join(".env"),
    ...
    fn already_configured_returns_file_values_and_saved_indicators() {
        let root = temp_root("darbot-already-configured");
        std::fs::create_dir_all(&root).unwrap();
        std::fs::write(
            root.join(".env"),
        ...
        std::fs::write(
            root.join(darbot_env::CHATGPT_STORE_FILE),
    ...
    fn already_configured_reports_legacy_anthropic_plan_without_returning_token() {
        let root = temp_root("darbot-already-configured-anthropic-session");
        std::fs::create_dir_all(&root).unwrap();
    ...
    fn passive_metadata_and_legacy_hints_are_root_and_provider_scoped() {
        ...
        for input in [
            ...
            if let Some(input) = input {
                std::fs::write(root.join(darbot_desktop_lib::saved_intent::FILE), input).unwrap();
            }
        ...
        std::fs::write(
            root.join(darbot_desktop_lib::saved_intent::FILE),
            r#"{"version":1,"categories":["intelligence","claude-plan"],"model":"claude-plan"}"#,
        )
        .unwrap();
    ...
    fn saved_selection_refusal_never_falls_back_to_a_different_provider_or_billing_mode() {
        ...
        for unreadable in [false, true] {
            if unreadable {
                std::fs::create_dir_all(root.join(darbot_env::CHATGPT_STORE_FILE)).unwrap();
            }
            let choice = ChosenModel {
    ...
    fn start_and_ask_resolve_saved_secrets_from_the_selected_root() {
        ...
        assert_eq!(
            credential,
            darbot_env::ModelCredential::OpenAi {
    ...
    fn saved_api_key_start_reports_unreadable_env_before_store_resolution() {
        ...
        assert_eq!(problem.said, "darbot could not read its settings.");
        assert!(
            problem
                .detail
                .as_deref()
    ...
    fn existing_installation_rejects_unusable_original_encryption_keys() {
        for marker in ["database", "model"] {
            ...
            if marker == "database" {
                ...
                .unwrap();
                assert!(darbot_desktop_lib::saved_intent::SavedIntent::read(&root)
                    .model
                    .is_some());
            }
            for original in [
    ...
    fn saved_api_key_selection_without_a_saved_key_is_rejected_before_starting_services() {
        for (provider, expected) in [
            ...
            let root = temp_root(&format!("darbot-missing-saved-{provider}"));
            std::fs::create_dir_all(&root).unwrap();
    ...
    fn saved_api_key_selection_uses_the_saved_key_when_it_still_exists() {
        for (provider, expected_key) in [
            ...
            let root = temp_root(&format!("darbot-present-saved-{provider}"));
            std::fs::create_dir_all(&root).unwrap();
            ...
            match credential {
                darbot_env::ModelCredential::OpenAi { api_key }
                | darbot_env::ModelCredential::Anthropic { api_key } => {
    ...
    fn persist_endpoint_fixture(root: &Path, credential: &darbot_env::ModelCredential) {
        let settings = darbot_env::compose(
            &darbot_env::Intelligence {
        ...
        let (public, secrets) = darbot_desktop_lib::vault::split(settings);
        darbot_desktop_lib::saved_intent::persist_configuration(
    ...
    fn stale_endpoint_hint_cannot_relabel_another_endpoints_stored_key() {
        ...
        std::fs::create_dir_all(&root).unwrap();
        let credential = darbot_env::ModelCredential::Compatible {
    ...
    fn compatible_endpoint_accepts_trimmed_container_url_and_rejects_invalid_one() {
        ...
        let darbot_env::ModelCredential::Compatible {
            base_url,
            container_base_url,
            model,
            ..
    ...
    fn responding_engine_without_compose_installs_then_redetects_before_returning() {
        ...
        let after = engine::EngineStatus {
            ...
            responding: true,
            engine_socket: None,
            detail: "podman is answering on darbot.".into(),
        };
        ...
        assert_eq!(ready.address.connection.as_deref(), Some("darbot"));
    }
    ...
    fn stop_shutdown_uses_the_active_root_at_the_external_command_boundary() {
        ...
        let active = temp_root("darbot-active-stop-root");
        let fallback = temp_root("darbot-default-stop-root");
        ...
        *shell.root.lock().unwrap() = Some(active.clone());
        ...
        let record = temp_root("darbot-stop-record").join("commands.log");
    ...
    fn quit_shutdown_uses_the_active_root_at_the_external_command_boundary() {
        ...
        let _path = SerializedPath::set();
        let active = temp_root("darbot-active-quit-root");
        let fallback = temp_root("darbot-default-quit-root");
        std::fs::create_dir_all(&active).unwrap();
        std::fs::write(active.join("docker-compose.yml"), "services: {}\n").unwrap();
        std::fs::create_dir_all(&fallback).unwrap();
        let shell = Shell::default();
        *shell.root.lock().unwrap() = Some(active.clone());
        ...
        let record = temp_root("darbot-quit-record").join("commands.log");
        let engine = fake_engine(&record);
        stack::down(&engine, &selected).expect("fake compose down");
    ...
    fn stop_reports_cleanup_and_down_failures_after_using_the_active_root() {
        let active = temp_root("darbot-active-stop-failures");
        let fallback = temp_root("darbot-default-stop-failures");
        std::fs::create_dir_all(&active).unwrap();
        std::fs::write(active.join("docker-compose.yml"), "services: {}\n").unwrap();
        std::fs::create_dir_all(&fallback).unwrap();
        let shell = Shell::default();
        *shell.root.lock().unwrap() = Some(active.clone());
        ...
        let problem = stop_everything_with(
            ...
            |root| {
                ...
                Err(Problem::with(
                    "darbot could not inspect or stop its host processes.",
        ...
        assert!(
            problem.contains("darbot could not inspect or stop its host processes."),
            "{problem}"
        );
    ...
    fn exit_cleanup_body_records_cleanup_and_down_failures_after_using_the_active_root() {
        let active = temp_root("darbot-active-exit-failures");
        let fallback = temp_root("darbot-default-exit-failures");
        std::fs::create_dir_all(&active).unwrap();
        std::fs::write(active.join("docker-compose.yml"), "services: {}\n").unwrap();
        std::fs::create_dir_all(&fallback).unwrap();
        let shell = Shell::default();
    ...
    fn production_stop_root_selection_retains_resolved_root_not_menu_fallback() {
        let selected = temp_root("darbot-production-stop-selected-root");
        let fallback = temp_root("darbot-production-stop-default-root");
        let shell = Shell::default();
    ...
    fn production_stop_root_selection_retains_stopped_selected_root_not_menu_fallback() {
        let selected = temp_root("darbot-production-stop-stopped-selected-root");
        let fallback = temp_root("darbot-production-stop-stopped-default-root");
        let shell = Shell::default();
    ...
    fn production_stop_root_selection_uses_menu_fallback_when_no_root_is_known() {
        let fallback = temp_root("darbot-production-stop-only-default-root");
        let shell = Shell::default();
    ...
    fn successful_stop_then_exit_uses_the_retained_selected_root_not_default() {
        let selected = temp_root("darbot-selected-stop-exit");
        let fallback = temp_root("darbot-default-stop-exit");
        std::fs::create_dir_all(&selected).unwrap();
        std::fs::create_dir_all(&fallback).unwrap();
...
fn main() {
    ...
    let record=PathBuf::from(env::var_os("darbot_TEST_ENGINE_RECORD").unwrap());
...
"#).unwrap();
                ...
                std::env::set_var("darbot_TEST_ENGINE_RECORD", base.join("commands.log"));
                let app = tauri::test::mock_builder()
                    .manage(Shell::default())
                    .invoke_handler(tauri::generate_handler![
                        start_stack,
    ...
    fn harness_start_ipc_case(case: &str) {
        ...
        let root = temp_root("darbot-harness-start-ipc");
        write_installed_deployment(&root);
        ...
        std::env::set_var("darbot_TEST_ENGINE_RECORD", &record);
        ...
        let settings = darbot_env::read_already_set(
            &root.join(".env"),
            &[
                "TENANT_PACKAGE_DIR",
                "MANAGED_AGENT_AG_UI_URL",
        ...
        if case.starts_with("remote") || case == "compatible" {
            ...
            assert!(request
                .headers
                .iter()
                .any(|line| line.starts_with("x-darbot-agent-token: ")));
            asked = true;
    ...
    fn start_fails_when_required_compose_service_exited_before_host_startup() {
        ...
        let root = temp_root("darbot-dead-compose-start");
        write_installed_deployment(&root);
        let record = temp_root("darbot-dead-compose-record").join("commands.log");
        ...
        assert_eq!(
            problem.said, "Part of darbot stopped during startup.",
    ...
    fn anthropic_start_does_not_raise_openai_only_agent_bot_or_fail_on_its_stale_exit() {
        ...
        let root = temp_root("darbot-anthropic-bot-selection-start");
        write_installed_deployment(&root);
        let record = temp_root("darbot-anthropic-bot-selection-record").join("commands.log");
        std::fs::create_dir_all(record.parent().expect("record parent")).unwrap();
        ...
        assert_eq!(problem.said, "Part of darbot stopped during startup.");
    ...
    fn ask_the_bot_uses_native_mastra_for_a_picked_mastra_harness() {
        ...
        let root = temp_root("darbot-mastra-ask");
        let answer = tauri::async_runtime::block_on(ask_the_bot_with_settings(
            ...
            std::collections::BTreeMap::from([
                ...
                ("PICKED_HARNESS_AGENT_ID".to_string(), "darbot".to_string()),
                (
    ...
    fn ask_the_bot_uses_the_picked_byo_ag_ui_endpoint_before_managed_fallback() {
        ...
        let root = temp_root("darbot-byo-ask");
        let answer = tauri::async_runtime::block_on(ask_the_bot_with_settings(
            root.clone(),
            "What is 17 times 23?".to_string(),
            std::collections::BTreeMap::from([
    ...
    fn ask_the_bot_keeps_body_read_errors_out_of_the_empty_answer_path() {
        ...
        let root = temp_root("darbot-body-read-ask");
    ...
    fn ask_command_attributes_empty_answers_to_the_selected_endpoint() {
        ...
        for case in [
            ...
            let installed = darbot_env::PickedHarness::Installed {
            ...
            let byo = darbot_env::PickedHarness::RemoteAgUi {
                url: format!("  {endpoint}  "),
                name: "An agent you already run".into(),
                remote_agent_id: String::new(),
            };
            let ports = darbot_env::Ports {
            ...
            let compose = |harness| {
                darbot_env::compose(
                    ...
                    &darbot_env::Model {
                        credential: darbot_env::ModelCredential::OpenAi {
            ...
            let write_settings = |values: &std::collections::BTreeMap<String, String>| {
                darbot_env::write(&file, values, &Default::default()).unwrap();
            };
            ...
            let public_settings = darbot_env::read_already_set(
                &file,
                &[
                    "PICKED_HARNESS_URL",
                    "PICKED_HARNESS_KIND",
            ...
            let request_correct = request.path
                ...
                && request
                    .headers
                    .iter()
                    .any(|header| header == "x-darbot-agent-token: synthetic-ask-token");
            std::fs::remove_dir_all(&root).unwrap();
            println!(
                "F5499_ASK_IPC={}",
                serde_json::json!({
    ...
    fn ask_the_bot_uses_managed_log_for_managed_fallback_empty_answer() {
        ...
        let record = temp_root("darbot-managed-empty-answer-record").join("commands.log");
        std::fs::create_dir_all(record.parent().expect("record parent")).unwrap();
        std::env::set_var("darbot_TEST_ENGINE_RECORD", &record);
        let server = TestServer::new(
            "HTTP/1.1 200 OK\r\ncontent-type: text/event-stream\r\nconnection: close\r\n\r\n\
        ...
        let root = temp_root("darbot-managed-empty-answer");
        std::fs::create_dir_all(&root).unwrap();
    ...
    fn ask_the_bot_keeps_harness_log_for_picked_harness_empty_answer() {
        ...
        let _path = SerializedPath::set_with("docker", "empty-answer");
        let record = temp_root("darbot-picked-empty-answer-record").join("commands.log");
        std::fs::create_dir_all(record.parent().expect("record parent")).unwrap();
        std::env::set_var("darbot_TEST_ENGINE_RECORD", &record);
        let server = TestServer::new(
            "HTTP/1.1 200 OK\r\ncontent-type: text/event-stream\r\nconnection: close\r\n\r\n\
        ...
        let root = temp_root("darbot-picked-empty-answer");
        std::fs::create_dir_all(&root).unwrap();
        ...
        let images = deployment::Images {
            ...
            images: std::collections::BTreeMap::from([
                (
                    "server".into(),
                    deployment::Image {
                        reference: "localhost/darbot-server@sha256:00".into(),
                    },
                ),
                (
                    "supervisor".into(),
                    deployment::Image {
                        reference: "localhost/darbot-supervisor@sha256:00".into(),
                    },
                ),
                (
                    "agent-computer".into(),
                    deployment::Image {
                        reference: "localhost/darbot-agent-computer@sha256:00".into(),
                    },
                ),
                (
                    "agent-bot".into(),
                    deployment::Image {
                        reference: "localhost/darbot-agent-bot@sha256:00".into(),
                    },
                ),
                (
                    "agent-langgraph".into(),
                    deployment::Image {
                        reference: "localhost/darbot-agent-langgraph@sha256:00".into(),
                    },
                ),
            ]),
        };
...
"#).unwrap();
        ...
        *shell.setup_url.lock().unwrap() = Some(setup.into());
        ...
        let show_error = invoke("show_darbot", serde_json::json!({})).unwrap_err();
        assert!(show_error.as_str().unwrap().contains("needs recovery"));
        restore_window_on(fixture.app.handle(), &darbot_env::Ports::default());
    ...
    fn recovery_notice_consumption_and_failed_retry_preserve_gate() {
        ...
        *shell.last_failure.lock().unwrap() = Some(Problem::plain("worker exhausted"));
        ...
        assert_eq!(notice.said, "darbot had trouble shutting down last time.");
        assert!(notice
            .detail
            .as_ref()
            .unwrap()
        ...
        assert!(show_darbot_on(app.handle().clone(), &f.ports).is_err());
    ...
    fn quit_notice_sink_failure_marks_recovery_and_shows_setup() {
        ...
        *shell.root.lock().unwrap() = Some(f.owned.clone());
        ...
        assert_eq!(failure.said, "darbot could not record a shutdown problem.");
        assert!(failure
            .detail
            .as_ref()
            .unwrap()
    ...
    fn failed_host_recording_retires_children_and_preserves_recording_failure() {
        ...
        assert_eq!(
            result.said,
            "darbot could not verify its host process ownership."
        );
        assert!(
            result
                .detail
    ...
    impl SerializedPath {
        ...
        fn set_with_path(binary: &str, scenario: &str, inherit_path: bool) -> Self {
            ...
            let previous_record = std::env::var_os("darbot_TEST_ENGINE_RECORD");
            let bin = temp_root("darbot-fake-engine-bin");
    ...
    impl Drop for SerializedPath {
        fn drop(&mut self) {
            ...
            if let Some(previous) = &self.previous_record {
                std::env::set_var("darbot_TEST_ENGINE_RECORD", previous);
            } else {
                std::env::remove_var("darbot_TEST_ENGINE_RECORD");
            }
        }
    }
...
"#).unwrap();
    ...
    fn restricted_engine_path_keeps_windows_credential_storage_available() {
        ...
        let path = SerializedPath::set_only_with("docker", "shutdown");
        let root = temp_root("darbot-restricted-path-credential-store");
        std::fs::create_dir_all(&root).unwrap();
        let saved = darbot_desktop_lib::vault::remember(
            &root,
            "OPENAI_API_KEY",
    ...
    fn setup_navigation_missing_main_config_is_reported() {
        ...
        assert_eq!(
            ...
            "the darbot setup window is not configured",
        );
    }
    ...
    fn app_adoption_and_restore_refuse_foreign_app_with_owned_api_still_running() {
        ...
        let app = f.app(&f.owned, "tauri://localhost/");
        let shown = show_darbot_on(app.handle().clone(), &f.ports);
        restore_window_on(app.handle(), &f.ports);
    ...
    fn app_adoption_and_restore_allow_owned_app_direct_and_launcher_descendant() {
        for descendant in [false, true] {
            ...
            let app = f.app(&f.owned, "tauri://localhost/");
            show_darbot_on(app.handle().clone(), &f.ports).unwrap();
            restore_window_on(app.handle(), &f.ports);
            assert_eq!(
                app.get_webview_window("main")
                    .unwrap()
