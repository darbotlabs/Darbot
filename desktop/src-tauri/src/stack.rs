...
/**
The Bots that ship with darbot, which only run on an API key.
...
pub const HOST_PROCESSES: [HostProcess; 3] = [
    ...
    // on darbotlm's "What's New" panel covering darbot entirely. An installed application should
...
/// was tested on, and would happen to anybody who installed darbot without also having Node.
...
///
/// Matching on this rather than on a name prefix. `darbot-` is also the prefix of a kind cluster's
/// nodes and of anything else somebody has called darbot, and stopping a person's Kubernetes
...
const SUPERVISOR_FILTER: &str = "label=darbot.supervisor=true";
...
pub fn stop_computers(engine: &Address, root: &Path) -> Result<bool, String> {
    ...
    let namespace_filter = format!("label=darbot.namespace={namespace}");
    let listed = engine
        .command()
        .args([
            "ps",
...
fn record_windows_host_processes_with(
    ...
    let problem = |detail| {
        Problem::with(
            "darbot could not verify its Windows host process ownership.",
            format!(
...
fn recorded_host_pid_file(root: &Path) -> Result<Option<RecordedHostPidFile>, Problem> {
    ...
    let problem = |detail| {
        Problem::with(
            "darbot could not read its recorded host processes.",
...
fn write_host_pid_file<T: Serialize>(root: &Path, value: &T) -> Result<(), Problem> {
    ...
    let problem = |operation: &str, error: &dyn std::fmt::Display| {
        Problem::with(
            "darbot could not record its host processes.",
...
fn configure_host_process_env(command: &mut Command, name: &str, secrets: &Secrets) {
    ...
    if name != "server" {
        command.env_remove("darbot_DESKTOP_HOST_TOKEN");
    }
}
...
pub fn replace_windows_host_process_with(
    ...
    let problem = |detail| {
        Problem::with(
            "darbot could not verify its Windows replacement process ownership.",
...
fn stop_windows_host_children_with(
    ...
    for (name, child) in children.iter_mut() {
        if child
            ...
            .map_err(|error| {
                Problem::with(
                    "darbot could not inspect a held host process.",
                    format!("{name}: {error}"),
                )
            })?
            .is_none()
    ...
    for (name, pid) in held {
        ...
        let record = live.and_then(|live| RecordedHostProcess::from_live(name, live))
            ...
            .ok_or_else(|| Problem::with(
                "darbot could not verify a held host process.",
...
pub fn stop_processes_under(_root: &Path) -> Result<usize, Problem> {
     ...
     * still answering 200 on 3010. Somebody who pressed Stop still had darbot serving.
     *
    ...
    let recorded = match recorded_host_pid_file(root)? {
        Some(RecordedHostPidFile::Pids(pids)) if !pids.is_empty() => {
            ...
            return Err(Problem::with(
                "darbot could not verify its recorded host processes.",
                format!(
...
fn stop_windows_processes_under_with(
    ...
    if let Some(record) = recorded.iter().find(|record| {
        ...
        return Err(Problem::with(
            "darbot could not verify one of its recorded host processes.",
            format!(
    ...
    match std::fs::remove_file(&path) {
        ...
        Err(error) => {
            return Err(Problem::with(
                "darbot could not remove its recorded host processes.",
    ...
    F: FnMut(u32) -> Result<bool, Problem>,
{
    let mut stopped = 0;
    let mut failures = Vec::new();
    for pid in verified_darbot_root_pids(recorded, processes) {
...
fn windows_processes_with(powershell: &Path) -> Result<Vec<WindowsProcess>, Problem> {
    ...
    if !output.status.success() {
        ...
        return Err(Problem::with(
            "darbot could not inspect its Windows host processes.",
...
/// Recorded darbot root processes whose live identity still matches the pid file.
pub fn verified_darbot_root_pids(
    recorded: &[RecordedHostProcess],
    processes: &[WindowsProcess],
) -> Vec<u32> {
    recorded
...
/// Recorded darbot processes, or their live children, listening on one of the host ports.
///
...
pub fn verified_darbot_pids_listening_on(
    listing: &str,
    ports: &[u16],
    recorded: &[RecordedHostProcess],
    processes: &[WindowsProcess],
) -> Vec<u32> {
    let roots = verified_darbot_root_pids(recorded, processes);
...
fn recorded_process_owns_port_windows_with(
    ...
    let verified = verified_darbot_pids_listening_on(&listed, &[port], &recorded, &processes);
    ...
    let output = compose_command(engine, root, &Secrets::new())
        ...
        .map_err(|error| {
            crate::problem::Problem::with(
                "darbot could not inspect its Compose services.",
                format!("could not run {operation}: {error}"),
            )
        })?;
...
"something is already listening on port 4206, which darbot uses for the Bot you picked" — about a
container darbot itself started, which the person never saw and cannot find. There is no way
...
pub fn port_already_taken_except(
    ...
    for (name, port) in ports {
        ...
        if something_answers(*port) {
            return Some(format!(
                "Something is already listening on port {port}, which darbot uses for the {name}. \
...
pub fn deployment_problem(root: &Path) -> Option<String> {
    if !root.exists() {
        return Some(format!(
            "{} does not exist yet. darbot needs a copy of the deployment there before it can \
             start one.",
            root.display()
        ));
    }
    if !root.join("docker-compose.yml").exists() {
        return Some(format!(
            "{} is not an darbot deployment: it has no docker-compose.yml.",
            root.display()
        ));
    }
...
fn missing_script(root: &Path) -> Option<String> {
     ...
     * this version of darbot", which sent somebody looking for a newer installer over a file with
    ...
    Some(format!(
        "The deployment in {} is older than this version of darbot: its app has no \"{APP_SCRIPT}\" \
         script, so there is no way to serve it. Install a newer darbot, or delete that directory \
...
mod tests {
    ...
    fn desktop_approval_transport_credential_reaches_only_the_server() {
        let secrets = Secrets::from([
            ("darbot_DESKTOP_HOST_TOKEN".into(), "fixture-only".into()),
        ...
        for name in ["server", "worker", "app"] {
            ...
            assert_eq!(
                vars[std::ffi::OsStr::new("darbot_DESKTOP_HOST_TOKEN")],
...
"#,
    ...
    fn windows_initial_inventory_does_not_own_a_direct_sibling_replacement() {
        ...
        assert_eq!(
            verified_darbot_root_pids(std::slice::from_ref(&original), &rows),
            [9000]
        );
        assert!(verified_darbot_pids_listening_on(
            listing,
            &[3001, 3010],
        ...
        assert_eq!(
            verified_darbot_pids_listening_on(
                listing,
                &[3001, 3010],
...
"#).unwrap();
    ...
    impl PathFixture {
        ...
        fn with_fake_engine_and_inherited_path(scenario: &str, inherit_path: bool) -> Self {
            ...
            let previous_scenario = std::env::var_os("darbot_FAKE_ENGINE_SCENARIO");
            let bin = temp_root("darbot-stack-fake-engine-bin");
            ...
            std::env::set_var("darbot_FAKE_ENGINE_SCENARIO", scenario);
            Self {
                previous,
                previous_record,
                previous_scenario,
    ...
    impl Drop for PathFixture {
        fn drop(&mut self) {
            ...
            if let Some(previous) = &self.previous_scenario {
                std::env::set_var("darbot_FAKE_ENGINE_SCENARIO", previous);
            } else {
                std::env::remove_var("darbot_FAKE_ENGINE_SCENARIO");
            }
...
fn main() {
    ...
    if let Ok(path) = std::env::var("darbot_TEST_ENGINE_RECORD") {
    ...
    let scenario = std::env::var("darbot_FAKE_ENGINE_SCENARIO").unwrap();
    if scenario == "computer-stop" {
        let root = std::path::PathBuf::from(std::env::var("darbot_TEST_ENGINE_RECORD").unwrap()).with_extension("");
            ...
            Some("ps") => {
                ...
                for (id, supervisor, namespace) in labels {
                    let matches = actual.windows(2).filter(|pair| pair[0] == "--filter").all(|pair| {
                        pair[1] == format!("label=darbot.supervisor={supervisor}")
                            || pair[1] == format!("label=darbot.namespace={namespace}")
                    });
...
"#;
    ...
    impl CleanupCommandFixture {
        fn new(root: &Path) -> Self {
            ...
            let bin = temp_root("darbot-cleanup-command-bin");
            std::fs::create_dir_all(&bin).unwrap();
...
"#;
    ...
    fn service_inspection_spawn_failure_is_a_problem() {
        ...
        let root = temp_root("darbot-service-inspection-spawn");
        std::fs::create_dir_all(&root).unwrap();
    ...
    fn service_inspection_nonzero_status_is_a_problem() {
        ...
        let root = temp_root("darbot-service-inspection-status");
        std::fs::create_dir_all(&root).unwrap();
    ...
    fn service_inspection_empty_success_is_healthy() {
        ...
        let root = temp_root("darbot-service-inspection-empty");
        std::fs::create_dir_all(&root).unwrap();
    ...
    fn service_inspection_blank_lines_are_healthy_empty_output() {
        ...
        let root = temp_root("darbot-service-inspection-blank-lines");
        std::fs::create_dir_all(&root).unwrap();
    ...
    fn malformed_service_inspection_rows_are_a_problem() {
        ...
        let root = temp_root("darbot-service-inspection-malformed");
        std::fs::create_dir_all(&root).unwrap();
    ...
    fn service_inspection_reports_only_unexpected_exited_services() {
        ...
        let root = temp_root("darbot-service-inspection-rows");
        std::fs::create_dir_all(&root).unwrap();
    ...
    fn source_bound_windows_command_failures_use_disposable_commands() {
        ...
        let root = temp_root("darbot-source-bound-windows-cleanup");
        std::fs::create_dir_all(&root).unwrap();
        let fixture = CleanupCommandFixture::new(&root);
    ...
    #[test]
    fn only_recorded_darbot_pids_are_selected_from_netstat_output() {
...
"#,
    ...
    fn recorded_server_ownership_requires_matching_identity_on_listening_port() {
        ...
        let root = temp_root("darbot-already-running-windows-owner");
        std::fs::create_dir_all(root.join(".logs")).unwrap();
        let fixture = CleanupCommandFixture::new(&root);
    ...
    fn recorded_server_ownership_is_false_without_current_records() {
        ...
        let root = temp_root("darbot-already-running-no-owner");
        std::fs::create_dir_all(root.join(".logs")).unwrap();
        let fixture = CleanupCommandFixture::new(&root);
        fixture.scenario("already-running");
        assert!(!recorded_process_owns_port_windows_with(
    ...
    fn a_reused_recorded_pid_is_not_selected_without_matching_identity() {
        ...
        let found =
            super::verified_darbot_pids_listening_on(listing, &[3001], &recorded, &processes);
    ...
    fn a_verified_recorded_host_keeps_its_listening_child_eligible_for_cleanup() {
        ...
        let roots = super::verified_darbot_root_pids(&recorded, &processes);
        let found =
            super::verified_darbot_pids_listening_on(listing, &[3010], &recorded, &processes);
    ...
    fn pidfile_readers_distinguish_missing_legacy_records_and_untrusted_evidence() {
        ...
        assert!(verified_darbot_root_pids(&legacy, &[live_process(42, 0, "created")]).is_empty());
    ...
    /// this version of darbot. That sent somebody looking for a newer installer over three bytes.
    #[test]
    ...
    fn a_missing_root_is_named_rather_than_left_to_errno() {
        let missing = std::env::temp_dir().join("darbot-not-here-at-all");
    ...
    fn a_deployment_directory_pasted_with_a_stray_space_is_the_one_it_names() {
        ...
        // from.
        assert_eq!(
            root_from("  /home/me/darbot  "),
            PathBuf::from("/home/me/darbot")
        );
        assert_eq!(
            root_from("/home/me/darbot\n"),
            PathBuf::from("/home/me/darbot")
        ...
        assert!(
            root_from(" /home/me/darbot").has_root(),
    ...
    fn a_space_inside_the_path_is_part_of_the_path() {
        ...
        // space in it must still be able to say where darbot lives.
        assert_eq!(
            root_from("/home/me/My Files/darbot"),
            PathBuf::from("/home/me/My Files/darbot")
        );
        assert_eq!(
    ...
    */
    ...
    #[test]
    fn the_bots_computers_are_found_by_label_rather_than_by_a_name_that_starts_with_darbot() {
        // A name filter would also match a kind cluster's nodes, which are called
        // darbot-control-plane and darbot-worker and belong to somebody else.
        assert!(
            SUPERVISOR_FILTER.starts_with("label="),
            "without this the engine answers `invalid filter`: {SUPERVISOR_FILTER}"
        );
        assert!(SUPERVISOR_FILTER.contains("darbot.supervisor=true"));
    ...
    fn computer_stop_filters_both_ownership_and_selected_namespace_for_each_engine() {
        ...
        for address in computer_stop_addresses(&path) {
            ...
            assert!(log.contains(&format!("{prefix}ps --quiet --filter label=darbot.supervisor=true --filter label=darbot.namespace=fixture-selected")), "{log}");
            assert!(
                log.lines()
    ...
    fn computer_stop_preserves_supervisor_default_and_trim_rules() {
        ...
        let path = PathFixture::with_fake_engine("computer-stop");
        for (index, namespace) in ["darbot", "", "  ", " fixture-selected "]
            .iter()
            .enumerate()
        {
            let (root, record) = computer_stop_root(
            ...
            .enumerate()
        {
            ...
            let log = std::fs::read_to_string(record).unwrap();
            assert!(
                log.contains(&format!("label=darbot.namespace={namespace}")),
                "{log}"
            );
...
"#,
    ...
    fn assert_windows_ancestry_selection(
        ...
        assert_eq!(
            verified_darbot_pids_listening_on(&listing, &[3010], recorded, processes),
            expected,
