...
//! When darbot installs Podman itself, the installer extends the *user's* PATH, and this process
...
//! command is therefore built from a resolved path, and the Compose provider darbot placed is put
...
/// Where darbot keeps the engine tools it installed itself.
///
...
/// Tell this module where the tools darbot installed live.
pub fn tools_live_in(dir: PathBuf) {
    let _ = TOOLS.set(dir);
}

/// The directory holding darbot's own copy of the Compose provider, under a download directory.
...
///
/// In front, so the provider darbot placed is the one found; appended, a broken `docker-compose`
...
/// choice is theirs. The fixed places are the fallback for the session in which darbot installed
...
fn where_installers_put(engine: Engine) -> Option<PathBuf> {
    let places: Vec<PathBuf> = match engine {
        ...
        Engine::Podman => {
            // The per-user MSI first: it is the one darbot runs. A machine-wide install left by
...
pub fn detect() -> EngineStatus {
    ...
    EngineStatus {
        ...
        responding: false,
        engine_socket: None,
        // Not an instruction any more: darbot installs one. See `install.rs`.
...
mod tests {
    ...
    fn a_podman_machine_is_named_on_every_command_it_is_addressed_with() {
        let address = Address::new(Engine::Podman, Some("darbot".into()));
        let command = address.command();
        let args: Vec<_> = command
            .get_args()
            .map(|arg| arg.to_string_lossy())
            .collect();
        assert_eq!(args, ["--connection", "darbot"]);
    }
    ...
    /// The program is a path, not a name. This is the fix for an engine darbot has just installed
    ...
    fn a_binary_that_is_nowhere_is_absent_rather_than_guessed_at() {
        assert_eq!(on_path("darbot-not-a-real-binary"), None);
    }
    ...
    fn the_provider_directory_goes_in_front_of_the_inherited_path() {
        let ours = Path::new("/tmp/darbot-tools");
        let joined = path_with(ours);
        let text = joined.to_string_lossy();
        assert!(text.starts_with("/tmp/darbot-tools"), "{text}");
        if let Some(existing) = std::env::var_os("PATH") {
    ...
    fn the_tools_live_under_the_downloads_they_came_from() {
        let downloads = Path::new("/tmp/darbot-engine");
        assert_eq!(tools_dir_under(downloads), downloads.join("bin"));
    }

    #[test]
    fn an_answering_engine_says_which_machine_answered() {
        let status = answering(Address::new(Engine::Podman, Some("darbot".into())));
        assert!(status.responding);
        assert!(status.detail.contains("darbot"), "{}", status.detail);
        assert_eq!(
