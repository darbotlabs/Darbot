...
#[cfg(test)]
const DEFAULT_IMAGE: &str = "darbot-agent-computer:s10-chromium-arm64";
...
impl Inner {
    ...
    fn spawn_operation(self: &Arc<Self>, client: Client, operation: DesktopOperation) {
        ...
        if let Err(error) = thread::Builder::new()
            .name("darbot-host-operation".into())
            .spawn(move || {
    ...
    fn write_file(&self, operation: &DesktopOperation) -> HostAccessResult<OperationSuccess> {
        ...
        let backup_path = format!("{APPROVED_MOUNT}/.darbot-backups/{backup_name}");
        let script = format!(
            "mkdir -p -- {backup_dir} && if [ -e {target} ]; then cp -- {target} {backup}; fi && cat > {target}",
            backup_dir = shell_quote(format!("{APPROVED_MOUNT}/.darbot-backups")),
        ...
        Ok(OperationSuccess::output(format!(
            "Wrote file. Backup, if the file existed, is .darbot-backups/{backup_name}"
        )))
    }
    ...
    fn verify_no_owned_containers(&self) -> HostAccessResult<()> {
        let filter = format!("label=darbot.host-access.instance={}", self.instance_label);
        let output = self
            .config
            .engine
            .command()
...
fn append_container_create_args(
    ...
    command.args(["--label", "darbot.host-access=true"]);
    command.arg("--label");
    command.arg(format!("darbot.host-access.instance={instance_label}"));
...
fn forbidden_paths_with_home(configured: &[PathBuf], home: Option<&Path>) -> ForbiddenPaths {
    ...
    if let Some(home) = home.and_then(|path| canonical(path).ok()) {
        ...
        for relative in [
            ...
            ".docker",
            ".kube",
            "Library/Application Support/darbot",
            "Library/Application Support/Google/Chrome",
            "Library/Application Support/BraveSoftware",
            "Library/Application Support/Firefox",
            "Library/Keychains",
            "AppData/Roaming/darbot",
...
fn fresh_container_name(operation_id: &str) -> String {
    let safe: String = operation_id
        ...
        .take(32)
        .collect();
    format!("darbot-host-{safe}-{}", std::process::id())
}
...
mod tests {
    ...
    fn ordinary_child_folder_under_home_is_allowed_but_home_itself_is_not() {
        ...
        let child = home.join(format!("darbot-host-access-{}", std::process::id()));
        fs::create_dir_all(&child).unwrap();
