...
pub fn compose(
    ...
    // machine that has never run darbot, which is exactly when generating is right.
    ...
    env.insert(
        "DATABASE_URL".into(),
        format!(
            "postgres://darbot:darbot@127.0.0.1:{}/darbot",
            ports.postgres
        ),
    );
    ...
    // The one person, named. `darbot_SINGLE_USER` says there is nobody else; this says who that
    // somebody is, so the routes that ask what an actor may do have an actor to answer about.
    env.insert("INITIAL_ADMIN_EMAILS".into(), "dev@darbot.local".into());
    ...
    env.insert("darbot_SINGLE_USER".into(), "true".into());
    ...
    // for `darbot-supervisor:latest`, which nobody publishes, and the denial that comes back
...
pub enum PickedHarness {
    Installed {
        /// The published image, e.g. `darbot-agent-crewai`. Named by the release, not derived.
        image: String,
...
const BANNER: &str = "# Written by darbot Desktop. Anything else in this file is left alone.";
...
const PUBLISHED: [&str; 4] = [
    ...
    "darbot-dev-supervisor-token",
    "darbot-dev-computer-token",
    "darbot-dev-worker-secret",
];
...
pub(crate) fn write_private_file(path: &Path, bytes: &[u8]) -> std::io::Result<()> {
    ...
    let temporary = parent.join(format!(".darbot-write-{:016x}.tmp", rand::random::<u64>()));
    let result = (|| {
...
pub fn write(
        ...
        .any(|(key, value)| key.contains(['\r', '\n']) || value.contains(['\r', '\n']))
    {
        return Err(std::io::Error::new(
            ...
            "darbot setting names and values must not contain line breaks.",
        ));
    }
    ...
    fn pinned() -> Vec<(String, String)> {
        crate::deployment::IMAGE_VARIABLES
            ...
            .map(|(published, variable)| {
                (
                    (*variable).to_string(),
                    format!("ghcr.io/darbotlm/darbot-{published}@sha256:abc"),
                )
            })
            .collect()
    }
...
"
    ...
    fn every_shared_secret_is_generated_rather_than_the_published_dev_default() {
        ...
        for key in [
            ...
            assert!(!value.contains("darbot-dev"), "{key} kept a dev default");
            assert!(
                value.len() > 20,
    ...
    */
    ...
    fn a_desktop_install_is_single_user_or_the_server_refuses_to_start() {
        ...
        assert_eq!(
            env.get("darbot_SINGLE_USER").map(String::as_str),
            Some("true")
        );
    }
    ...
    fn a_published_value_is_replaced_rather_than_carried_forward() {
        ...
        std::fs::write(
            ...
            "KEY_ENCRYPTION_KEY=AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=\n\
             SUPERVISOR_TOKEN=darbot-dev-supervisor-token\n\
             COMPUTER_TOKEN=darbot-dev-computer-token\n\
             WORKER_SHARED_SECRET=darbot-dev-worker-secret\n",
        )
        .unwrap();
        ...
        for (mastra, expected, port, run_path, url) in [
            ...
            let env = compose(
                ...
                Some(&PickedHarness::Installed {
                    image: "darbot-agent-crewai".into(),
                    port,
                    name: "CrewAI".into(),
                    mastra,
