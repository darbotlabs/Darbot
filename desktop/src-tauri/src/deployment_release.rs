...
const LATEST_RELEASE: &str = "https://api.github.com/repos/darbotlm/darbot/releases/latest";
...
pub fn resolve_version(root: &Path) -> Result<String, String> {
    resolve_version_with(root, || {
        let body = deployment::get(LATEST_RELEASE)
            .map_err(|error| format!("could not find the latest darbot release: {error}"))?;
        release_tag(&body)
    })
}
...
fn release_tag(body: &[u8]) -> Result<String, String> {
    ...
    let release: Release = serde_json::from_slice(body)
        .map_err(|error| format!("the latest darbot release is not readable: {error}"))?;
    if release.tag_name.trim().is_empty() {
        return Err("the latest darbot release has no version tag".into());
    }
    Ok(release.tag_name)
}
...
mod tests {
    ...
    fn githubs_exact_tag_is_used_instead_of_the_release_title() {
        assert_eq!(
            release_tag(br#"{"name":"darbot September release","tag_name":"v0.0.9"}"#).unwrap(),
            "v0.0.9"
        );
    }
