...
//! output alone is what put "pull access denied for darbot-agent-langgraph-agui, repository does
...
pub fn said_about(output: &str) -> String {
    ...
    {
        return "darbot could not download one of the parts it needs. That version may not have \
                been published yet. Check for an darbot update, and try again."
            .into();
    }
    if lower.contains("port is already allocated") || lower.contains("address already in use") {
        return "Something else on this computer is using a port darbot needs. Close it, or \
                restart the computer, and try again."
            .into();
    }
    if lower.contains("no space left") {
        return "This computer has run out of disk space, so darbot could not finish. Free some \
                space and try again."
            .into();
    }
    ...
    {
        return "darbot cannot reach the container engine. Start Docker or Podman, wait for it to \
    ...
    if lower.contains("unauthorized") || lower.contains("permission denied") {
        return "darbot was refused permission for something it needed. The details below say \
                what, and are worth sending to whoever set this up."
            .into();
    }

    "Something went wrong while setting darbot up. The details below are worth sending to \
     whoever set this up."
        .into()
}
...
mod tests {
    ...
    fn a_refused_pull_reads_as_a_missing_release_not_a_login_problem() {
        let raw =
            "Error response from daemon: pull access denied for darbot-agent-langgraph-agui, \
        ...
        assert!(!said.contains("darbot-agent"), "{said}");
    }
    ...
    fn a_taken_port_says_so_in_words_somebody_can_act_on() {
        ...
        assert!(said.contains("port darbot needs"), "{said}");
    }
