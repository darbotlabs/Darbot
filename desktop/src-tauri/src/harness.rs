...
//!
//! Two things this list deliberately does not contain. darbot's own `built-in` agent type, which
...
/// repository and its history is largely darbotlm's own people. It means the model vendor does
/// not maintain it.
...
/// Mastra server, and darbot dials it through `getRemoteAgents` from `@ag-ui/mastra`, the bridge
...
/// `registerdarbotlm` in the harness amounted to: that route serves the darbotlm Runtime
...
pub fn catalogue() -> Vec<Harness> {
     ...
     *
     * A release publishes `darbot-<directory>`, taken from the Dockerfile paths in the tree, so
     * the image name belongs to the directory and not to whatever this list calls the row. Derived
     * from the id it was wrong for every row — `darbot-harness-crewai` against a published
     * `darbot-agent-crewai` — and wrong twice for the four whose id does not match their folder.
    ...
    let ours = |id: &str,
        ...
        // The manifest's own key, which is the directory the image is built from. `darbot-` is
...
mod tests {
    ...
    /// darbot's own `built-in` agent type is a system prompt, not a harness, and the doc is
    ...
    */
    ...
    fn deployment_naming_everything(label: &str) -> std::path::PathBuf {
        ...
        let named: Vec<String> = catalogue()
            ...
            .map(|image| {
                format!(
                    "\"{image}\": {{ \"repository\": \"ghcr.io/darbotlm/darbot-{image}\", \
                     \"digest\": \"sha256:abc\", \
                     \"reference\": \"ghcr.io/darbotlm/darbot-{image}@sha256:abc\" }}"
                )
            })
            .collect();
        std::fs::write(
    ...
    fn write_crewai_manifest(root: &std::path::Path) {
        std::fs::write(
            ...
            "{ \"version\": \"v9.9.9\", \"images\": { \
             \"agent-crewai\": { \
             \"reference\": \"ghcr.io/darbotlm/darbot-agent-crewai@sha256:abc\" } } }",
        )
    ...
    fn start_fetches_deployment_before_resolving_a_selected_harness_image() {
        ...
        assert_eq!(image, "ghcr.io/darbotlm/darbot-agent-crewai@sha256:abc");
    ...
    fn mastra_resolves_as_mastra_and_names_its_agent() {
        ...
        assert!(mastra);
        assert_eq!(remote_agent_id, "darbot");
        let _ = std::fs::remove_dir_all(&root);
    }
    ...
    `darbot-agent-langgraph-agui:v0.0.8` to `docker.io/library/...` and the person was told access
    ...
    */
    ...
    /// Mastra is offered, and the row is the assertion that the bridge on darbot's side works.
