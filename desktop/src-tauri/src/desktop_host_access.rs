...
use darbot_desktop_lib::host_access::{
...
impl<R: tauri::Runtime> NativeApproval<R> {
    fn confirm(&self, title: &str, message: String) -> HostAccessResult<()> {
        let window = self
            ...
            .ok_or_else(|| refused("The local darbot window is closed."))?;
...
impl<R: tauri::Runtime> HostApprovalUi for NativeApproval<R> {
    fn choose_folder(&self, request: &ChooseFolderPrompt) -> HostAccessResult<ApprovedFolder> {
        ...
        self.confirm("Allow folder access?", format!(
            "Bot: {bot}\nRequested by: {}\n\nFolder: {}\n\nAllow this Bot to read this folder for this darbot session? Each edit and command asks separately. You can revoke access in Computers.",
    ...
    fn confirm_write(&self, request: &WritePrompt) -> HostAccessResult<()> {
        ...
        self.confirm("Allow this file change?", format!(
            "Bot: {bot}\nFolder: {}\nFile: {}\n\nNew content:\n{}\n\nAllow this exact change once? darbot keeps the previous contents when replacing a file.",
