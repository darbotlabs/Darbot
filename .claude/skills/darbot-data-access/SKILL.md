...
5. **The pending branch renders nothing.** darbot has no loading placeholder — no "Loading…"
...
- The data arrives over the darbotlm runtime → not this skill; `lib/copilot/`.
...
| `<PageEmpty>Loading …</PageEmpty>`, a spinner, or a `Skeleton` while a query is pending | darbot uses no loading placeholder; the flicker costs more than the reassurance buys | Return `null` from the pending branch |
...
| A read type carrying a token, key, or `plaintext` | Secrets are write-only in darbot | Expose `hasAuth: boolean` or a `revokedAt` timestamp |
