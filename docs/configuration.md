...
darbot is configured with environment variables and a tenant package. The API server validates both at startup.
...
| `INTELLIGENCE_API_URL`        | darbotlm Intelligence API URL.                                                                      |
| `INTELLIGENCE_GATEWAY_WS_URL` | darbotlm Intelligence realtime gateway URL.                                                         |
...
`darbotlm_LICENSE_TOKEN` is optional: managed Intelligence issues no licence token, and a
...
| `NODE_ENV`           | unset                              | `production` refuses the example `KEY_ENCRYPTION_KEY`. It does not decide whether sign-in is required; see `darbot_SINGLE_USER`. |
...
| `darbot_GENERATIVE_UI` | unset (capability on)               | Set `false` or `0` to stop Bots from answering with generated interfaces. |

**`darbot_GENERATIVE_UI`** enables generated interfaces by default: streamed HTML/CSS/JavaScript
...
Set `darbot_GENERATIVE_UI=false` or `0` to disable both. `true`, `1`, an empty value, or an unset
...
local default, `darbot-dev-worker-secret`, the same value every clone of this repository gets. That
...
| Variable                     | Meaning                                                                                |
| ---------------------------- | -------------------------------------------------------------------------------------- |
| `darbot_SINGLE_USER`        | One fixed administrator and no sign-in. **Required** when no identity provider is configured, or the deployment refuses to start. Ignored when one is. |
...
| `darbot_PUBLIC_URL`         | Public address of this API. Defaults to `BETTER_AUTH_URL`.                              |
| `darbot_APP_URL`            | Where the browser app is served. Defaults to the first `TRUSTED_ORIGINS` entry.          |

**With no provider at all, `darbot_SINGLE_USER=true` is required.** A deployment that configures
...
are covered in [charts/darbot/README.md](../../charts/darbot/README.md). The similarly named
...
```sh
docker ps -aq --filter "label=darbot.namespace=darbot" | xargs -r docker rm -f
```
...
`COMPUTER_NAMESPACE` defaults to `darbot` and names the deployment a computer belongs to. It is part
...
```sh
docker ps -aq --filter "label=darbot.namespace=darbot" | xargs -r docker rm -f
docker volume ls -q --filter "label=darbot.namespace=darbot" | xargs -r docker volume rm
```
...
| ----------------- | ------------------- | ------------------------------------------ |
| `agent-computer`  | `COMPUTER_IMAGE`    | `ghcr.io/darbotlm/darbot-agent-computer` |
| `supervisor`      | `SUPERVISOR_IMAGE`  | `ghcr.io/darbotlm/darbot-supervisor`     |
| `agent-bot`       | `BOT_IMAGE`         | `ghcr.io/darbotlm/darbot-agent-bot`      |
| `agent-langgraph` | `LANGGRAPH_IMAGE`   | `ghcr.io/darbotlm/darbot-agent-langgraph`|
| `migrate`         | `SERVER_IMAGE`      | `ghcr.io/darbotlm/darbot-server`         |
...
Set `darbot_ONE_COMPUTER_EACH=false` when using `start.sh` to run all Bots against one shared computer.
