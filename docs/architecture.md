...
| `server`                 | 3001                       | API, darbotlm runtime, auth, roles, tenant package, coworkers, channels, policy, audit, credentials, plugins, components, and connectors. |
...
| darbotlm Intelligence  | external                   | Durable threads, memory, and realtime gateway.                                                                                              |
...
3. darbotlm runtime sends the turn to the configured AG-UI endpoint.
...
allow rule does not permit. darbot's shipped startup default is explicit:
...
A channel is a conversation with one coworker and a darbotlm Intelligence thread mapping. Starting a new channel creates a new thread.
...
- With no identity provider configured, the deployment refuses to start unless `darbot_SINGLE_USER=true` says every request may be one fixed administrator. That flag is the only thing that permits it; `NODE_ENV` does not.
