...
### 1. Enable the connector in darbot
...
`darbot_PUBLIC_URL` if it is set, or the auth base URL it falls back to otherwise — nothing needs to
...
else here. Unlike Google Drive's tool list, which is darbot's own code and needs no credential to
...
press **Connect**. That leaves darbot for Notion's own consent screen — the arrow on the button says
...
- [Configuration](../configuration.md) — `darbot_PUBLIC_URL`, `darbot_APP_URL`, `KEY_ENCRYPTION_KEY`.
