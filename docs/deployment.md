...
darbot ships as one container. It carries the app, the API that serves it, and the browser the Bots
...
image=ghcr.io/darbotlm/darbot:latest
...
docker run -p 3001:3001 --env-file .env \
  -e EMBEDDED_POSTGRES=on -v darbot-data:/var/lib/postgresql "$image"
```

`docker build -t darbot .` from a clone produces the same thing, for anyone deploying a tree of
...
| `INTELLIGENCE_API_URL`, `INTELLIGENCE_GATEWAY_WS_URL`, `INTELLIGENCE_API_KEY` | darbotlm Intelligence. A free plan is available and it can be self-hosted |
| `darbotlm_LICENSE_TOKEN` | optional. Managed Intelligence issues none; set it only for a self-hosted Intelligence that has one |
...
Configure Google, Microsoft or Okta, or set `darbot_SINGLE_USER=true` to say you meant an open
...
```sh
docker run --rm --env-file .env --entrypoint sh darbot \
...
started everything in the image. Without the override, `docker run … darbot sh -c "… migrate.ts"`
...
[charts/darbot/README.md](../charts/darbot/README.md), which covers EKS, GKE, AKS and a plain
