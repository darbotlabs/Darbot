...
beforeAll(async () => {
  ...
  for (const [id, email] of [
    [askerId, `${askerId}@darbot.test`],
    [otherId, `${otherId}@darbot.test`],
  ]) {
    await database
      .insert(users)
...
describe("retiring the credentials a person owns", () => {
  ...
  test("a credential orphaned by a deleted user row is still retired", async () => {
    ...
    await store.retireConnectionsFor(otherId, "admin@darbot.local");
  ...
  test("retiring twice is quiet, and nobody owns nothing", async () => {
    ...
    expect(
      (await store.retireConnectionsFor("", "admin@darbot.local")).retired,
    ).toBe(0);
  });
});
