...
describe("agent lifecycle routes", () => {
  test("attaches authentication middleware to every route before calling the store", async () => {
    ...
    for (const [path, init] of requests) {
      const response = await app.request(`http://darbot.test${path}`, init);
  ...
  test("uses only the authenticated context actor and parses hidden as exact true", async () => {
    ...
    for (const query of [
      ...
      expect((await app.request(`http://darbot.test/${query}`)).status).toBe(
        200,
      );
    }
  ...
  test("serves every lifecycle route with its contract status and store operation", async () => {
    ...
    const list = await app.request("http://darbot.test/");
    const detail = await app.request("http://darbot.test/agent-1");
    const created = await app.request("http://darbot.test/", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(validInput),
    });
    const updated = await app.request("http://darbot.test/agent-1", {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(validInput),
    });
    ...
    const hidden = await app.request("http://darbot.test/agent-1/hide", {
      method: "POST",
    });
    const unhidden = await app.request("http://darbot.test/agent-1/unhide", {
      method: "POST",
    });
    const deleted = await app.request("http://darbot.test/agent-1", {
      method: "DELETE",
    });
  ...
  test("projects exact DTO fields and computes permissions for the authenticated actor", async () => {
    ...
    const response = await appFor(store).request("http://darbot.test/");
  ...
  test("separates ownership from permission for an administrator", async () => {
    ...
    const body = (await json(
      await appFor(store, requireAdministrator).request("http://darbot.test/"),
  ...
  test("never forwards forged create or update fields", async () => {
    ...
    for (const [path, method] of [
      ...
      const response = await app.request(`http://darbot.test${path}`, {
        method,
        headers: { "content-type": "application/json" },
  ...
  test.each([
    ...
    const store = fakeStore();
    const app = appFor(store);
    const malformed = await app.request(`http://darbot.test${path}`, {
      method,
      headers: { "content-type": "application/json" },
      body: "{",
    });
    const partial = await app.request(`http://darbot.test${path}`, {
      method,
      headers: { "content-type": "application/json" },
  ...
  test("returns 404 when get returns null", async () => {
    ...
    const response = await appFor(store).request("http://darbot.test/missing");
...
describe("agent route composition", () => {
  test("mounts the store behind createApp authentication with the derived actor", async () => {
    ...
    const unauthenticated = await app.request("http://darbot.test/api/agents");
    ...
    const authenticated = await app.request("http://darbot.test/api/agents");
  ...
  test("leaves agent routes unmounted when createApp has no store", async () => {
    ...
    const response = await app.request("http://darbot.test/api/agents");
