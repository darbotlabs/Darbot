...
function appWith(
  ...
  return (body: unknown) =>
    app.request("http://darbot.test/api/plugins/servers", {
      method: "POST",
...
describe("granting one Bot to another", () => {
  test("an administrator can grant it", async () => {
    ...
    const response = await app.request(
      "http://darbot.test/api/plugins/grants",
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
  ...
  test("and revoke it again", async () => {
    ...
    const response = await app.request(
      "http://darbot.test/api/plugins/grants?kind=bot&ref=knowledge&agentId=assistant",
      { method: "DELETE" },
    );
