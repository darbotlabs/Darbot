...
function navigate(
  ...
  return routes.request("http://darbot.test/bot-9/navigate", {
    method: "POST",
...
describe("the frame a page was opened on", () => {
  ...
  test("the kept frame is read back by the page it was taken of", async () => {
    ...
    const response = await routes.request(
      "http://darbot.test/bot-9/page-frame/call-1",
    );
  ...
  test("a turn nobody photographed reads back as no frame", async () => {
    ...
    const response = await routes.request(
      "http://darbot.test/bot-9/page-frame/call-never",
    );
  ...
  test("a deployment keeping no frames still navigates", async () => {
    ...
    expect(
      await (
        await routes.request("http://darbot.test/bot-9/page-frame/call-1")
      ).json(),
    ).toEqual({ frame: null });
  });
});
