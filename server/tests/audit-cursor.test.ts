...
describe("audit cursor validation", () => {
  test("a corrupt cursor is a query error, not a server failure", () => {
    expect(() =>
      auditQueryFromUrl(
        new URL("http://darbot.local/api/admin/audit-events?cursor=!!bogus!!"),
      ),
    ).toThrow(AuditQueryError);
    expect(() =>
      auditQueryFromUrl(
        new URL(
          "http://darbot.local/api/admin/audit-events?cursor=bm90LWpzb24=",
        ),
      ),
  ...
  test("a well-formed cursor still parses", () => {
    const query = auditQueryFromUrl(
      new URL(
        `http://darbot.local/api/admin/audit-events?cursor=${validCursor()}&limit=10`,
      ),
    );
  ...
  test("the admin route answers 400 for a corrupt cursor instead of 500", async () => {
    ...
    const response = await app.request(
      "http://darbot.local/api/admin/audit-events?cursor=!!bogus!!",
    );
  ...
  test("the admin route still pages with a valid cursor", async () => {
    ...
    const response = await app.request(
      `http://darbot.local/api/admin/audit-events?cursor=${validCursor()}`,
    );
