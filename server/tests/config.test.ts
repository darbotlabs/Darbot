...
describe("deployment configuration", () => {
  ...
  test.each([
    ...
    expect(() => loadConfig(environment)).toThrow(
      `darbotlm Intelligence is required and is not configured. Missing: ${name}`,
    );
  });

  test("starts without darbotlm_LICENSE_TOKEN, because managed Intelligence no longer issues one", () => {
    const environment: Record<string, string | undefined> = {
      ...baseEnvironment,
    };
    delete environment.darbotlm_LICENSE_TOKEN;
  ...
  test("still forwards a licence token when a deployment sets one", () => {
    const config = loadConfig({
      ...baseEnvironment,
      darbotlm_LICENSE_TOKEN: "self-hosted-licence",
    });
  ...
  test("refuses to start when Intelligence is absent entirely, rather than degrading", () => {
    ...
    ).toThrow("darbotlm Intelligence is required and is not configured");
  });
  ...
  test("enables Google authentication when its complete deployment contract is present", () => {
    const config = loadConfig({
      ...
      BETTER_AUTH_SECRET: "a-long-enough-local-development-auth-secret",
      BETTER_AUTH_URL: "http://localhost:3001",
      INITIAL_ADMIN_EMAILS: "admin@darbot.test, owner@darbot.test",
    });
    ...
    expect(config.auth).toEqual({
      ...
      initialAdminEmails: ["admin@darbot.test", "owner@darbot.test"],
    });
  });
  ...
  const OPEN = { darbot_SINGLE_USER: "true" };
...
describe("accessibility", () => {
  ...
  test.each(["true", "1"])(
    "is off on darbot_ACCESSIBILITY_DISABLED=%p",
    (value) => {
      expect(
        loadConfig({
          ...baseEnvironment,
          darbot_ACCESSIBILITY_DISABLED: value,
        }).accessibility,
      ).toBe(false);
    },
  );
  ...
  test.each(["false", "no", "", "yes"])(
    "stays on for darbot_ACCESSIBILITY_DISABLED=%p",
    (value) => {
      expect(
        loadConfig({
          ...baseEnvironment,
...
describe("generated interfaces", () => {
  ...
  test.each(["true", "1"])("stay on for darbot_GENERATIVE_UI=%p", (value) => {
    expect(
      loadConfig({ ...baseEnvironment, darbot_GENERATIVE_UI: value })
        .generativeUi,
    ).toBe(true);
  });

  test.each(["false", "0"])("are off for darbot_GENERATIVE_UI=%p", (value) => {
    expect(
      loadConfig({ ...baseEnvironment, darbot_GENERATIVE_UI: value })
        .generativeUi,
    ).toBe(false);
  ...
  test.each(["no", "", "yes", "TRUE", "on"])(
    "stay on for darbot_GENERATIVE_UI=%p",
    (value) => {
      expect(
        loadConfig({ ...baseEnvironment, darbot_GENERATIVE_UI: value })
          .generativeUi,
  ...
  test("ignore the disable switch this replaced", () => {
    expect(
      loadConfig({
        ...baseEnvironment,
        darbot_GENERATIVE_UI_DISABLED: "false",
      }).generativeUi,
    ).toBe(true);
  });
});
