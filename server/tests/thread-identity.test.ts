...
const here = createThreadIdentity("darbot-production");
const elsewhere = createThreadIdentity("darbot-staging");
...
describe("a thread id carries the deployment that minted it", () => {
  ...
  test("the same name recognises threads minted by another process", () => {
    ...
    expect(createThreadIdentity("darbot-production").owns(here.mint())).toBe(
      true,
    );
  });
