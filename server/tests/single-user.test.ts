...
describe("running with no sign-in", () => {
  ...
  test("NODE_ENV decides nothing, in either direction", () => {
    ...
    expect(
      singleUserEnabled(
        { NODE_ENV: "production", darbot_SINGLE_USER: "true" },
        false,
      ),
    ).toBe(true);
  });
  ...
  test("a flag set to anything but true is not a yes", () => {
    // Read exactly, so `darbot_SINGLE_USER=false` and a stray `1` both refuse rather than opening.
    for (const value of ["false", "1", "yes", "TRUE", ""]) {
      expect(() =>
        singleUserEnabled({ darbot_SINGLE_USER: value }, false),
      ).toThrow();
    }
  });
});
