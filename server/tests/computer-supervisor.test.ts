...
describe("locating a Bot's computer", () => {
  test("uses the address the supervisor reports", async () => {
    const client = clientWith(() =>
      Response.json({
        botId: "sales",
        container: "darbot-computer-sales",
        status: "running",
        url: "http://darbot-computer-sales:4100",
      }),
    );
    expect(await client.locate("sales")).toBe(
      "http://darbot-computer-sales:4100",
    );
  });
        ...
        fetchImpl: (async () =>
          Response.json({
            computers: [
              {
                botId: "bot",
                container: "darbot-computer-bot",
                status: dockerStatus,
                url: "http://darbot-computer-bot:4100",
              },
            ],
          })) as unknown as typeof fetch,
      });
...
describe("telling one run of a computer from the next, across replicas", () => {
  ...
  function replica(botId: string, seen: string[] = []) {
    const running = [
      {
        botId,
        container: `darbot-computer-${botId}`,
        status: "running",
        url: `http://darbot-computer-${botId}:4100`,
        startedAt,
      },
    ];
    return createDockerSupervisorProvider({
