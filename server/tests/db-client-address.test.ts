 ...
 * socket, so `postgres://…/darbot` cannot connect on Windows (oven-sh/bun#27713); and it prefers
...
describe("the database address", () => {
  ...
  test("refuses a connection string that is not a URL", () => {
    expect(() => createDatabase("://darbot@/darbot")).toThrow(
  ...
  test("does not put the password in the message when the URL will not parse", () => {
    ...
    let message = "";
    try {
      createDatabase(`postgres://darbot:${secret}@127.0.0.1:notaport/darbot`);
    } catch (error) {
  ...
  test("refuses a URL with no host, which would otherwise parse and connect nowhere", () => {
    // `new URL` accepts this: the scheme is "darbot:" and there is no host at all.
    expect(() => createDatabase("darbot:darbot@localhost/darbot")).toThrow(
      /names no host/,
    );
  });
  ...
  test("refuses a URL that names no database, rather than connecting to a default", () => {
    expect(() =>
      createDatabase("postgres://darbot:darbot@127.0.0.1:5432"),
  ...
  test("refuses a port of zero instead of connecting nowhere", () => {
     ...
     */
    expect(() =>
      createDatabase("postgres://darbot:darbot@127.0.0.1:0/darbot"),
  ...
  test("still accepts a password that IS percent-encoded, decoding it", () => {
    ...
    expect(() =>
      createDatabase("postgres://darbot:p%40ss@127.0.0.1:5432/darbot"),
    ).not.toThrow();
  });
