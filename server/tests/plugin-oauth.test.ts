...
describe("the address the person is sent to", () => {
  ...
  const url = new URL(
    authorizationUrlFor({
      auth: googleAuth,
      clientId: "client-id",
      redirectUri: "https://darbot.example/api/plugins/oauth/callback",
      state: "signed-state",
...
describe("the address the vendor sends them back to", () => {
  test("is one path, built from the deployment's own public URL", () => {
    expect(redirectUriFor("https://darbot.example")).toBe(
      "https://darbot.example/api/plugins/oauth/callback",
    );
  });
  ...
  test("registers a dynamic client with the redirect URI and no secret", async () => {
    ...
    try {
      ...
      expect(seen[0]?.body).toEqual({
        redirect_uris: ["https://darbot.example/api/plugins/oauth/callback"],
    ...
    try {
      expect(
        await registerDynamicClient({
          registrationUrl: "https://vendor.example/register",
          redirectUri: "https://darbot.example/cb",
        }),
      ).toBeNull();
    } finally {
      globalThis.fetch = realFetch;
