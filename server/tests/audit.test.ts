...
const memberAuth = {
  ...
  api: {
    getSession: async () => ({
      user: { id: "member", email: "member@darbot.test" },
    }),
  },
};
...
describe("admin audit API", () => {
  test("returns a filtered audit page to an administrator", async () => {
    ...
    const response = await app.request(
      "http://darbot.local/api/admin/audit-events?eventType=connector.sync_succeeded&limit=10",
    );
  ...
  test("denies a non-admin caller", async () => {
    ...
    const response = await app.request(
      "http://darbot.local/api/admin/audit-events",
    );
