...
afterEach(async () => {
  await database.transaction(async (tx) => {
    await tx.execute(
      sql`select set_config('darbot.audit_retention_days', '1', true)`,
    );
      ...
      expect(
        await refusedAsAppendOnly(() =>
          database.transaction(async (tx) => {
            await tx.execute(
              sql`select set_config('darbot.audit_retention_days', ${window}, true)`,
            );
            await tx
              .delete(auditEvents)
              .where(eq(auditEvents.targetType, MARKER));
  ...
  test("a recent row cannot be deleted, whatever the window says", async () => {
    ...
    expect(
      await refusedAsAppendOnly(() =>
        database.transaction(async (tx) => {
          await tx.execute(
            sql`select set_config('darbot.audit_retention_days', '3650', true)`,
          );
          await tx
            .delete(auditEvents)
            .where(eq(auditEvents.targetType, MARKER));
  ...
  test("a declared retention window does not open the door to it", async () => {
    ...
    expect(
      await refusedAsAppendOnly(() =>
        database.transaction(async (tx) => {
          await tx.execute(
            sql`select set_config('darbot.audit_retention_days', '30', true)`,
          );
          await tx.execute(sql`truncate table audit_events`);
          throw new Error(ROLLED_BACK);
        }),
