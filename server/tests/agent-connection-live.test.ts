 ...
 *
 * `@darbotlm/aimock` is ours, which is the point: it is the org's deterministic backend for
 * exactly this, it tracks the protocol as the protocol moves, and using it here means darbot finds
...
describe("registering an agent that answers badly", () => {
  ...
  test("a tool call in the stream is reported, because that is what a governed Bot does", async () => {
    /*
     * The event that matters most for this product. A remote Bot yields a tool call and darbot
