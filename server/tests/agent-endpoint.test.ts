...
describe("dialling a stored agent endpoint", () => {
  ...
  const runRequest = {
    ...
    body: JSON.stringify({
      threadId: "t",
      forwardedProps: { darbotBotId: "risk", darbotRun: "signed.run.token" },
    }),
  };
  ...
  test("a hop to another host does not take the signed run with it", async () => {
    ...
    expect(forwarded.forwardedProps?.darbotRun).toBeUndefined();
    // Only the credential is removed. The rest of the run is still the run.
    expect(forwarded.threadId).toBe("t");
    expect(forwarded.forwardedProps?.darbotBotId).toBe("risk");
  });
  ...
  test("a hop that stays on the same host keeps them", async () => {
    ...
    expect(forwarded.forwardedProps?.darbotRun).toBe("signed.run.token");
  });
