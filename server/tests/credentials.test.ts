...
describe("admin credential API", () => {
  test("returns only credential status and metadata", async () => {
    ...
    const response = await app.request(
      "http://darbot.local/api/admin/credentials",
    );
  ...
  test("rotates and revokes through write-only administrator operations", async () => {
    ...
    const rotate = await app.request(
      "http://darbot.local/api/admin/credentials/credential-old/rotate",
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
    ...
    const revoke = await app.request(
      "http://darbot.local/api/admin/credentials/credential-new/revoke",
      { method: "POST" },
    );
