...
/**
 * The darbotlm runtime, always in Intelligence mode.
 *
 * Package-declared built-in Bots run as darbotlm `BuiltInAgent` instances. External Bots are
 ...
 *
 * The darbot equivalent of a CLAUDE.md, and the third instruction carrier beside the two that
...
function remoteAgentWithStandingRole(
  ...
  const runWith = (
    ...
    const forwardedProps = {
       ...
       */
      darbotDeploymentTools: deploymentTools,
      /*
...
const RESERVED_MASTRA_CONTEXT_DESCRIPTIONS = new Set([
  ...
  "darbot Bot id",
  "darbot deployment tools",
  "darbot signed run assertion",
]);
...
class CloningRemoteAgent extends AbstractAgent {
  ...
  constructor(
    remote: AbstractAgent,
    private readonly attachdarbotMiddleware: (target: AbstractAgent) => void,
  ) {
    super({
    ...
    this.attachdarbotMiddleware(this);
  }
...
/**
 * Mount the darbotlm endpoint onto the host Hono app.
 *
 ...
 * example — but it is not re-exported from `@darbotlm/runtime/v2`, and the package's `exports` map
 ...
 */
class IntelligenceKnowingANewThread extends darbotlmIntelligence {
  override getThreadMessages(
    params: Parameters<darbotlmIntelligence["getThreadMessages"]>[0],
  ) {
...
export function mountCopilotRuntime(
  ...
  const runtime = new CopilotRuntime({
    ...
    // Carried on the events the runtime already sends, so darbot's traffic is separable from any
    // other deployment's. Adds no events of its own.
    telemetryProperties: {
      ...(config.accessibility ? { accessibility_title: "darbot" } : {}),
  ...
  return {
    ...
    history: async (input: { threadId: string; actorId: string }) => {
       ...
       */
      type Read = Awaited<
        ReturnType<darbotlmIntelligence["getThreadMessages"]>
      >;
