  ...
  routes.post("/servers/:id/connect", requireUser, async (context) => {
    ...
    if (!connect?.publicUrl) {
      return context.json(
        {
          error:
            "This deployment has no public URL configured, so it cannot complete a consent flow. Set darbot_PUBLIC_URL.",
        },
        503,
      );
    }
     ...
     * without darbot_PUBLIC_URL — the redirect URI it registers is guaranteed to exist.
     */
    /*
