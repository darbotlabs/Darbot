...
export function createApp(
  ...
  /**
   * The darbotlm endpoint, already built by the caller.
   *
   ...
   * scope broke every server test that touches createApp even though none of them use darbotlm.
   */
  ...
  // The darbotlm runtime, behind the same session guard as every other API route. Mounted last so
  // its own routing under /api/darbotlm cannot shadow an darbot route declared above.
  if (copilotHandler) {
    // Mounted at the ROOT with the handler carrying its own basePath. Mounting it at
    // "/api/darbotlm" as well double-prefixes it: Hono strips the prefix before the handler sees
    // the path, so every route lands at /api/darbotlm/api/darbotlm/* and /info 404s. The browser
  ...
  if (pluginStore || deploymentToolCaller) {
    ...
    app.post("/api/agent-tools/call", async (context) => {
      ...
      const verdict = await authoriseAgentCall({
        presented: context.req.header("x-darbot-agent-token") ?? "",
        run: body?.run,
        encryptionKey: config.keyEncryptionKey,
        legacyToken,
        lookup: async (hash) =>
