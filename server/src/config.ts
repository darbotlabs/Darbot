/**
 * What the runtime can do. There is exactly one answer because darbotlm Intelligence is required
 ...
 * `@darbotlm/runtime` declares `licenseToken` optional with a `darbotlm_LICENSE_TOKEN` fallback
...
export type DeploymentConfig = {
   ...
   *
   * `darbot_PUBLIC_URL` when set, otherwise `BETTER_AUTH_URL`, which is the same public address for
   ...
   *
   * `darbot_APP_URL` when set, otherwise the first `TRUSTED_ORIGINS` entry, which is already defined
   ...
   */
  singleUser: boolean;
  /** Names darbot on the analytics the runtime already sends. Off with darbot_ACCESSIBILITY_DISABLED. */
   ...
   * darbot_GENERATIVE_UI=false or darbot_GENERATIVE_UI=0.
   *
 ...
 * were a choice.
 *
 * darbotlm_LICENSE_TOKEN IS NO LONGER ONE OF THEM. Managed Intelligence issues a single project
...
function runtimeCapabilities(environment: Environment): RuntimeCapabilities {
  const settings = {
    ...
    licenseToken: optional(environment, "darbotlm_LICENSE_TOKEN"),
  };
  ...
  if (missing.length > 0) {
    throw new Error(
      `darbotlm Intelligence is required and is not configured. Missing: ${missing.join(", ")}`,
    );
  }
  ...
  if (sandboxNamespace) {
    return {
      ...
      templateFile:
        ...
        "/etc/darbot/sandbox-template.json",
      allowPrivateHosts,
...
function accessibilityEnabled(environment: Environment): boolean {
  const off = optional(environment, "darbot_ACCESSIBILITY_DISABLED");
...
function generativeUiEnabled(environment: Environment): boolean {
  const value = optional(environment, "darbot_GENERATIVE_UI");
...
export function loadConfig(
  ...
  return {
    ...
    publicUrl: (
      optional(environment, "darbot_PUBLIC_URL") ?? auth?.baseUrl
    )?.replace(/\/+$/, ""),
    appUrl: (
      optional(environment, "darbot_APP_URL") ??
      commaSeparated(environment, "TRUSTED_ORIGINS")[0] ??
      optional(environment, "darbot_PUBLIC_URL") ??
      auth?.baseUrl
    )?.replace(/\/+$/, ""),
