...
/**
 * Who is asking, for a darbotlm request.
 *
...
async function resolveRequestActor(request: Request): Promise<{
  ...
  if (!user) {
    throw new Error("A darbotlm run requires a signed-in user.");
  }
  const roles = await roleRepository.rolesForUser(user.id);
  if (!roles.includes("admin") && !roles.includes("user")) {
    throw new Error("A darbotlm run requires an authorized user.");
  }
  return {
    id: user.id,
 ...
 */
const routineIntelligence = new darbotlmIntelligence({
...
const app = createApp(
  ...
  hostAccessBroker,
  process.env.darbot_DESKTOP_HOST_TOKEN,
...
console.info(`darbot server listening on http://127.0.0.1:${port}`);
