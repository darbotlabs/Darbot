...
Generative UI is enabled by default; set `darbot_GENERATIVE_UI=false` or `0` to disable it.
...
of a refusal in the container's log and nowhere else. The sentence on screen is darbot's own and
...
### Signing in to darbotlm from the window works
...
nothing for anybody who already had darbot.
...
would go on, and four more for navigating between setup and darbot. macOS authorizes every
...
darbot itself had started, which the person never saw and could not find, and there was no way
...
the manifest unreadable and was reported as "the deployment is older than this version of darbot",
sending somebody looking for a newer installer over three bytes.

### The installed app is served without a development server

"Show darbot" did nothing on a machine where the stack was up. The window said darbot was running,
...
button also shows what it was told: the call behind it already answered "darbot is not answering on
...
from darbot's own database, so a channel is listed whatever the history store says, while the
...
The desktop setup screen asks where darbot should live, enables Start once that box is not blank
...
darbot, so an unrelated process on 3010 is named and left alone rather than killed. Nothing is
...
copies of "Written by darbot Desktop" and fifty blank lines stacked above its settings. The banner
...
session of an installed darbot met a vault it could no longer read: every stored credential failed
...
### A desktop shell that installs darbot and then becomes it
...
secrets that are darbot's to generate, raises the containers, applies the migrations, installs the
dependencies, starts the host processes, waits until both the API and the app answer, and then shows
darbot in the same window. It is not a launcher for a browser tab. Stop takes the stack down, closing
...
because none of them is something darbot can fix on somebody's behalf.
...
holding a `%` that starts no escape -- `postgres://darbot:100%pure@host:5432/darbot`, which a
...
Two things stopped darbot running on Podman, which nothing had tried before.
...
This applies to the five `ghcr.io/darbotlm/darbot-<service>` images and not to
`ghcr.io/darbotlm/darbot`. Reading a zstd layer needs a client that supports it, which Podman and
...
`ghcr.io/darbotlm/darbot` was the only image a release produced, so anything running the Compose
...
Each of those is now published beside it, at `ghcr.io/darbotlm/darbot-<service>`, carrying both
...
`darbotlm_LICENSE_TOKEN` is no longer required. Managed Intelligence derives entitlement from the
...
read instead of look at. Set `darbot_GENERATIVE_UI=true` and it may answer with an interface it
...
same container: it could `psql -h 127.0.0.1 -U darbot` with no password and read the audit trail,
...
`docker run -v darbot-data:/var/lib/postgresql` case, which relied entirely on that hidden chown.
...
Reported by [@jerelvelarde](https://github.com/darbotlm/darbot/issues/269) with the container logs
...
neither darbotlm Intelligence, nor an identity provider, nor a Bot: nobody could sign in and no
...
A Helm chart under `charts/darbot`, Bots and all, and the fixes that installing it for real turned
...
Ingress rather than instead of it. `charts/darbot/ci` holds a values file per target.
...
the gateway on `5432` accepted a connection and began authenticating as `darbot` on `darbot`, with
...
### `start.sh` refuses a port that answers but is not darbot
...
`darbot-dev-*` defaults and exports them before calling compose. A plain `docker compose up -d` —
...
- No provider at all and no `darbot_SINGLE_USER=true`. Configure a provider, or set that to say you
...
- **This deployment does not search documents itself.** A Bot answers from a live system by calling
  ...
  a copy of a customer's corpus is not a thing darbot does.
...
- **A Bot can answer from Google Drive, as the person asking.** Ask a Bot a question whose answer is
  ...
  Google with a message that never mentions darbot. See
...
- **Releases are cut by a workflow, not by hand.** `Create release PR` bumps the version and promotes
  ...
  builds and pushes one image to `ghcr.io/darbotlm/darbot`, signs a build provenance attestation
...
- **Registering a company's identity provider was owned by whoever registered it.** Better Auth
  ...
  registered is a fact about the deployment, so reads and removals go through darbot's own
...
- **`allowed_groups` is documented as a declaration, not a control.** The tenant package writes it and
  ...
  for. Thanks to [@NathanTarbert](https://github.com/darbotlm/darbot/pull/92) and
  [@andreolf](https://github.com/darbotlm/darbot/issues/82).
- **Running with no sign-in takes a flag and nothing else.** It used to be locked with
  ...
  deployment with no provider now refuses to start unless `darbot_SINGLE_USER=true` says it was
  meant. `.env.example` ships that line switched on, so a clone still runs with no configuration at
  all, and the line is greppable in a way a default never was. `darbot_DEV_NO_AUTH` is still
  honoured.
...
- **Where a Bot's computer runs is now a plug.** One `ComputerProvider` interface sits under the
  ...
  [@mu-hashmi](https://github.com/darbotlm/darbot/pull/57) for the refactor.
