...
import { builddarbotInstructions, darbotBaseInstructions } from "./index";
  ...
  const child = Bun.spawn(
    [
      ...
      [
        ...
        'const model = mastra.getAgent("darbot").model;',
...
describe("darbot Mastra receiver instructions", () => {
  test("adds model-visible darbot role context in receiver order", () => {
    const instructions = builddarbotInstructions({
  ...
  test("keeps ordinary Mastra calls on the base receiver instruction", () => {
    expect(builddarbotInstructions()).toBe(darbotBaseInstructions);
    expect(
      builddarbotInstructions({
        requestContext: requestContextWith("not ag-ui context entries"),
      }),
    ).toBe(darbotBaseInstructions);
  });
});

describe("darbot Mastra model configuration", () => {
...
describe("darbot Mastra listen port configuration", () => {
