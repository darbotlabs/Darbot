 ...
 *
 * These wrap the canonical primitives from `@darbotlm/shared` (the same
 * ones `@darbotlm/core` uses for its `FrontendTool` parameters) plus a
 * couple of Slack-local conveniences.
 */
import { schemaToJsonSchema as sharedSchemaToJsonSchema } from "@darbotlm/shared";
import type { StandardSchemaV1, InferSchemaOutput } from "@darbotlm/shared";
 ...
 * objects (and `@darbotlm/core` constrains tool args to
