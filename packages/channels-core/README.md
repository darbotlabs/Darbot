# @darbotlm/channels-core

The supported platform-neutral foundation behind `@darbotlm/channels`.
Most applications should use the batteries-included `@darbotlm/channels` package;
...
```sh
pnpm add @darbotlm/channels-core @darbotlm/channels-slack
```
...
```json
{
  "compilerOptions": {
    "jsx": "react-jsx",
    "jsxImportSource": "@darbotlm/channels-core"
  }
}
```

```ts
import { createChannel } from "@darbotlm/channels-core";
...
See `@darbotlm/channels-slack` for a complete implementation.
...
`@darbotlm/channels-ui` vocabulary.
