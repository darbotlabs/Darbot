...
runtime, no Slack — `@darbotlm/channels-ui` depends on nothing in the repo
except `@darbotlm/shared` (for `StandardSchemaV1` types). That's what lets
a platform adapter (e.g. `@darbotlm/channels-slack`) translate the same UI into
...
for the engine (`@darbotlm/channels`) to bind.

## Install

```sh
pnpm add @darbotlm/channels-ui
```
...
```jsonc
{
  "compilerOptions": {
    "jsx": "react-jsx",
    "jsxImportSource": "@darbotlm/channels-ui",
  },
}
```

This package ships `@darbotlm/channels-ui/jsx-runtime` (and
...
// ir is BotNode[] — hand it to an adapter, or let @darbotlm/channels post it.
```
...
action binding and re-render rehydration possible in `@darbotlm/channels`.
...
(resolved via `jsxImportSource: "@darbotlm/channels-ui"`). So JSX is statically
...
they're implemented at runtime by `@darbotlm/channels` and its adapters.
`@darbotlm/channels-ui` has no runtime dependency on them.
...
```tsx
import { bind } from "@darbotlm/channels-ui";
