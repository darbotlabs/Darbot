...
import {
  createVectorRequestHandler,
  type VectorState,
} from "./vector-handler.js";
...
export class VectorMock implements Mountable {
  ...
  async handleRequest(
    ...
    if (handled && this.registry) {
      const { operation, provider } = classifyVectorRequest(
        req.method ?? "GET",
        pathname,
      );
        ...
        req.on("end", () => {
          ...
          try {
            ...
            if (req.method !== "GET") {
              ...
              res.writeHead(400, { "Content-Type": "application/json" });
              res.end(
                JSON.stringify({ error: `Malformed JSON body: ${detail}` }),
              );
              return;
            }
          }
...
function classifyVectorRequest(
  ...
  if (
    /^\/collections\/[^/]+\/points\/delete$/.test(pathname) &&
    method === "POST"
  ) {
  ...
  // ChromaDB paths
  if (
    /^\/api\/v1\/collections\/[^/]+\/query$/.test(pathname) &&
    method === "POST"
  ) {
    return { operation: "query", provider: "chromadb" };
  }
  if (
    /^\/api\/v1\/collections\/[^/]+\/add$/.test(pathname) &&
    method === "POST"
  ) {
