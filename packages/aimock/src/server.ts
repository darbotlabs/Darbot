...
function handleNotFound(res: http.ServerResponse, message: string): void {
  ...
  writeErrorResponse(
    res,
    404,
    JSON.stringify({ error: { message, type: "not_found" } }),
  );
}
...
export function performFullReset(
  ...
  if (targets.defaults.registry) {
    targets.defaults.registry.setGauge(
      "aimock_fixtures_loaded",
      {},
      fixtures.length,
    );
  }
...
async function handleControlAPI(
  ...
  if (subPath === "/fixtures" && req.method === "POST") {
    ...
    try {
      ...
      defaults.logger.error(
        `POST /__aimock/fixtures: failed to read body: ${msg}`,
      );
  ...
  if (subPath === "/error" && req.method === "POST") {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      defaults.logger.error(
        `POST /__aimock/error: failed to read body: ${msg}`,
      );
      res.writeHead(400, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ error: `Failed to read request body: ${msg}` }));
      return true;
...
async function handleCompletions(
  ...
  const openRouterFallback =
    openRouter && Array.isArray(body.models) && body.models.length > 0;
  ...
  if (openRouterFallback) {
    ...
    const resolveCandidate = async (
      ...
      skippedBySequenceOrTurn = Math.max(
        skippedBySequenceOrTurn,
        attempt.skippedBySequenceOrTurn,
      );
    ...
    for (const candidate of candidates) {
      ...
      if (outcome.isError) {
        ...
        const errorFallsThrough =
          (outcome.response as ErrorResponse).fallthrough !== false;
        ...
        chaosAction === "malformed"
          ? {
              ...
              onHookBypassed: (
                ...
                defaults.registry?.incrementCounter(
                  "aimock_chaos_bypassed_total",
                  {
                    action: "malformed",
                    source: "proxy",
                    reason,
  ...
  const response =
    preResolvedResponse ?? (await resolveResponse(fixture, body));
  ...
  const shapeORChunks = (
    ...
    const shaping = resolveOpenRouterShaping(
      overrides,
    ...
    if (usageChunk)
      shapeOpenRouterChunks([usageChunk], shaping, overrides?.id !== undefined);
  };
  ...
  if (isContentWithToolCallsResponse(response)) {
    ...
    if (!streaming) {
      ...
      const completionText = streamingBlocks
        ? streamingBlocks
            .map((b) => (b.type === "text" ? b.text : b.name + b.arguments))
            .join("")
        : (response.content ?? "") +
          (response.toolCalls ?? [])
  ...
  if (isToolCallResponse(response)) {
    ...
    if (body.stream !== true) {
      ...
      const completionText = response.toolCalls
        .map((tc) => tc.name + tc.arguments)
        .join("");
      const usageChunk = emitStreamingUsage
        ? buildUsageChunk(
...
export async function createServer(
  ...
  const resolvedAuth = resolveInboundAuth({
    value: options?.auth,
    label: "options.auth",
  });
  return createServerWithResolvedAuth(
    fixtures,
    options,
    resolvedAuth,
    mounts,
...
export async function createServerWithResolvedAuth(
  ...
  if (options?.chaos) {
    ...
    for (const { name, value } of chaosRates) {
      if (value !== undefined && (value < 0 || value > 1)) {
        logger.warn(
          `Chaos ${name} (${value}) is outside 0-1 range — will be clamped at runtime`,
        );
      }
    }
  }
  ...
  for (const { name, config } of [
    ...
    for (const field of [
      ...
      if (!Number.isFinite(value)) {
        logger.warn(
          `${name}.${field} (${value}) is not a finite number — treating as unset`,
        );
  ...
  const server = http.createServer(
    (req: http.IncomingMessage, res: http.ServerResponse) => {
      ...
      handleHttpRequest(req, res).catch((err: unknown) => {
        ...
        const url = req.url ?? "?";
        defaults.logger.warn(
          `Unhandled request error on ${method} ${url}: ${msg}\n${stack}`,
        );
        if (!res.headersSent) {
  ...
  async function handleHttpRequest(
    ...
    const openRouterVideoContentMatch = pathname.match(
      OPENROUTER_VIDEO_CONTENT_RE,
    );
    ...
    if (pathname === OPENROUTER_VIDEO_MODELS_PATH && req.method === "GET") {
      try {
        await handleOpenRouterVideoModels(
          req,
          res,
          fixtures,
          journal,
    ...
    const openRouterVideoStatusMatch = pathname.match(
      OPENROUTER_VIDEO_STATUS_RE,
    );
    ...
    if (pathname === EMBEDDINGS_PATH && req.method === "POST") {
      try {
        ...
        const embeddingsProvider: RecordProviderKey = azureDeploymentId
          ? "azure"
          : "openai";
    ...
    if (
      grokVideoStatusMatch &&
      grokVideoStatusMatch[1] !== "generations" &&
      req.method === "GET"
    ) {
      try {
    ...
    if (pathname === GEMINI_INTERACTIONS_PATH && req.method === "POST") {
      try {
        const raw = await readBody(req);
        await handleGeminiInteractions(
          req,
          res,
          raw,
          fixtures,
    ...
    if (
      pathname === ELEVENLABS_SOUND_GENERATION_PATH &&
      req.method === "POST"
    ) {
      setCorsHeaders(res);
      try {
  ...
  async function handleUpgradeRequest(
    ...
    if (pathname === RESPONSES_PATH) {
      ...
      const transcriptionIntent =
        parsedUrl.searchParams.get("intent") === "transcription";
