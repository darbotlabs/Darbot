...
export class AGUIMock implements Mountable {
  ...
  async handleRequest(
    ...
    if (this.registry) {
      this.registry.incrementCounter("aimock_agui_requests_total", {
        method: "POST",
      });
    }
    ...
    try {
      ...
      res.end(
        JSON.stringify({ error: `Failed to read request body: ${detail}` }),
      );
    ...
    if (fixture) {
      await writeAGUIEventStream(res, fixture.events, {
  ...
  async start(): Promise<string> {
    ...
    return new Promise<string>((resolve, reject) => {
      const srv = http.createServer(async (req, res) => {
        ...
        const handled = await this.handleRequest(req, res, url.pathname).catch(
          (err) => {
            this.logger.error(
              `AGUIMock request error: ${err instanceof Error ? err.message : err}`,
            );
            if (!res.headersSent) {
              res.writeHead(500);
