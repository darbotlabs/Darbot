...
export async function startFromConfig(
  ...
  if (
    replaySpeed != null &&
    (!Number.isFinite(replaySpeed) || replaySpeed <= 0)
  ) {
    logger.warn(
      `llm.replaySpeed must be a positive number, got ${replaySpeed}. Ignoring.`,
    );
    ...
    if (mcpConfig.prompts) {
      for (const prompt of mcpConfig.prompts) {
        ...
        if (result) {
          mcp.addPrompt(
            def,
            () => result as import("./mcp-types.js").MCPPromptResult,
          );
        } else {
          mcp.addPrompt(def);
        }
