...
export class A2AMock implements Mountable {
  private agents: Map<
    string,
    { def: A2AAgentDefinition; patterns: PatternEntry[] }
  > = new Map();
  ...
  async handleRequest(
    ...
    if (req.method === "GET" && pathname === "/.well-known/agent-card.json") {
      if (this.registry) {
        this.registry.incrementCounter("aimock_a2a_requests_total", {
          method: "GetAgentCard",
        });
      }
  ...
  private async handleStreamingMessage(
    ...
    const userParts: A2APart[] = params?.message
      ? (((params.message as Record<string, unknown>).parts as A2APart[]) ?? [
          { text },
        ])
      : [{ text }];
