...
export function audioToFalFile(
  ...
  if (typeof response.audio === "string") {
    data = response.audio;
    contentType =
      FORMAT_TO_CONTENT_TYPE[response.format ?? "mp3"] ?? "audio/mpeg";
  } else {
...
export async function handleFalQueue(
  ...
  // Unknown fal path
  const errorBody = {
    error: { message: "Unknown fal.ai endpoint", type: "not_found" },
  };
  journal.add({
...
async function tryRecordAudioQueueWalk(args: {
  ...
  if (!finalBody || typeof finalBody !== "object") {
    ...
    res.end(
      JSON.stringify({
        error: {
          message: "Upstream result body was not a JSON object",
          type: "proxy_error",
        },
      }),
    );
