...
function buildBlockList(): BlockList {
  ...
  for (const [addr, prefix] of PRIVATE_V4_RANGES)
    bl.addSubnet(addr, prefix, "ipv4");
  for (const [addr, prefix] of PRIVATE_V6_RANGES)
    bl.addSubnet(addr, prefix, "ipv6");
  return bl;
}
...
async function resolveHttpFixture(
  ...
  try {
    ...
    opts.logger.info(
      `Fetched ${url} (${body.length} bytes) → cached at ${cacheFile}`,
    );
    ...
    if (opts.validateOnLoad) {
      throw new Error(
        `Failed to fetch ${url} and no cached copy available: ${msg}`,
      );
    }
    opts.logger.warn(
  ...
  try {
    ...
    if (len) {
      ...
      if (Number.isFinite(n) && n > maxBytes) {
        throw new Error(
          `response too large: content-length ${n} exceeds limit ${maxBytes} bytes`,
        );
      }
    }
    ...
    if (!res.body) {
      ...
      if (Buffer.byteLength(text, "utf-8") > maxBytes) {
        throw new Error(
          `response too large: body exceeds limit ${maxBytes} bytes`,
        );
      }
      return text;
    }
