 ...
 *
 * Usage:
 *   import { useAimock } from "@darbotlm/aimock/vitest";
 *
...
export function useAimock(options: UseAimockOptions = {}): () => AimockHandle {
  ...
  afterAll(async () => {
    if (handle) {
      if (options.patchEnv !== false) {
        if (origOpenaiUrl !== undefined)
          process.env.OPENAI_BASE_URL = origOpenaiUrl;
        else delete process.env.OPENAI_BASE_URL;
        if (origAnthropicUrl !== undefined)
          process.env.ANTHROPIC_BASE_URL = origAnthropicUrl;
  ...
  return () => {
    if (!handle) {
      throw new Error(
        "useAimock(): server not started — are you calling this inside a test?",
      );
    }
    return handle;
  };
