...
function resolveChaosConfig(
  ...
  if (fixture?.chaos) {
    if (fixture.chaos.dropRate !== undefined)
      base.dropRate = fixture.chaos.dropRate;
    if (fixture.chaos.malformedRate !== undefined)
      base.malformedRate = fixture.chaos.malformedRate;
  ...
  if (rawHeaders) {
    ...
    if (typeof dropHeader === "string") {
      ...
      if (isNaN(val)) {
        logger?.warn(
          `[chaos] x-aimock-chaos-drop: invalid value "${dropHeader}", ignoring`,
        );
      } else {
        if (val < 0 || val > 1) {
          logger?.warn(
            `[chaos] x-aimock-chaos-drop: value ${val} out of range [0,1], clamping`,
          );
        }
  ...
  if (base.dropRate !== undefined)
    base.dropRate = Math.min(1, Math.max(0, base.dropRate));
...
export function evaluateChaos(
  ...
  if (
    config.dropRate !== undefined &&
    config.dropRate > 0 &&
    Math.random() < config.dropRate
  ) {
    return "drop";
  }
  if (
...
export function applyChaosAction(
  ...
  if (registry) {
    registry.incrementCounter("aimock_chaos_triggered_total", {
      action,
      source,
    });
  }
