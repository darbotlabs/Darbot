...
export class MCPMock implements Mountable {
  ...
  private resources: Map<
    string,
    { def: MCPResourceDefinition; content?: MCPResourceContent }
  > = new Map();
  private prompts: Map<
    string,
    {
  ...
  onToolCall(
    ...
    handler: (
      args: unknown,
    ) => MCPContent[] | string | Promise<MCPContent[] | string>,
  ): this {
  ...
  async handleRequest(
    ...
    if (this.registry) {
      if (req.method === "DELETE") {
        this.registry.incrementCounter("aimock_mcp_requests_total", {
