...
export async function handleBedrock(
  ...
  if (isContentWithToolCallsResponse(response)) {
    if (response.webSearches?.length) {
      logger.warn(
        "webSearches in fixture response are not supported for Bedrock API — ignoring",
      );
    }
...
function parseToolArgumentsForStream(
  toolCall: ToolCall,
  logger: Logger,
): string {
  try {
...
export function buildBedrockStreamTextEvents(
  ...
  events.push(
    buildBedrockInvokeMessageDelta(
      bedrockStopReason(overrides?.finishReason, "end_turn"),
    ),
  );
