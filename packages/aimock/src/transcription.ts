...
export function extractFormField(
  ...
  if (!boundary) {
    ...
    console.warn(
      "extractFormField: no multipart boundary found, using best-effort regex fallback",
    );
...
export async function handleTranscription(
  ...
  const responseFormat =
    extractFormField(raw, "response_format", boundary) ?? "json";
  ...
  const useVerbose =
    responseFormat === "verbose_json" || t.words != null || t.segments != null;
