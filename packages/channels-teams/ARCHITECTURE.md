...
Application authors use `@darbotlm/channels-teams` with the product-facing
[`@darbotlm/channels`](../channels) umbrella. `TeamsAdapter` imports and
implements [`PlatformAdapter`](../channels-core) from
[`@darbotlm/channels-core`](../channels-core) to plug Microsoft Teams into the
platform-agnostic channel engine, exactly as
[`@darbotlm/channels-slack`](../channels-slack) does for Slack. You write the
...
[`@darbotlm/channels-core`](../channels-core): ingress normalization, egress
...
```
TeamsAdapter (`@darbotlm/channels-teams`)
  └── imports / implements ──► `@darbotlm/channels-core`: `PlatformAdapter`

`@darbotlm/channels` is the product-facing umbrella, not an adapter dependency.
```
...
Teams ──HTTP──▶ POST /api/messages (listener.ts, express)
              ...
              ▼
          handleActivity (adapter.ts; `PlatformAdapter` from `@darbotlm/channels-core`)
