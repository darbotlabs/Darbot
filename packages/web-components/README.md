...
Angular, or `@darbotlm/core` code and renders correctly in any host page,
surviving hostile host CSS (`all: unset`, Tailwind preflight `!important`).

## Install

```bash
npm install @darbotlm/web-components lit
```
...
```ts
import { defineCopilotKitThreadsDrawer } from "@darbotlm/web-components/threads-drawer";
