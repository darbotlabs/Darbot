...
import { ɵInterruptState } from "@darbotlm/core";
import type { ɵPendingInterrupt } from "@darbotlm/core";
...
export interface UseInterruptInChatConfig<TValue = unknown, TResult = never>
  extends UseInterruptConfigBase<TValue, TResult> {
  /** When true (default), the interrupt UI renders inside `<CopilotChat>` automatically. */
  renderInChat?: true;
}

export interface UseInterruptExternalConfig<TValue = unknown, TResult = never>
  extends UseInterruptConfigBase<TValue, TResult> {
  /** When false, the hook returns the interrupt element so you can place it yourself. */
  renderInChat: false;
}

export interface UseInterruptDynamicConfig<TValue = unknown, TResult = never>
  extends UseInterruptConfigBase<TValue, TResult> {
