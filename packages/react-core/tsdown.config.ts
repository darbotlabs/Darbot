...
// relative ../context imports to the external @darbotlm/react-core/v2/context
  ...
  const walk = (current: string) => {
    for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        ...
        const next = code
          .replace(cssImport, "")
          .replace(contextImport, 'from "@darbotlm/react-core/v2/context"');
...
// src/v2/context.ts onto the external `@darbotlm/react-core/v2/context`
// package path.
//
...
// copy while consumers importing from `@darbotlm/react-core/v2/context` read
  ...
  resolveId(source: string, importer?: string) {
    ...
    if (importer && /context(\.ts)?$/.test(source)) {
      ...
      if (
        ...
        return {
          id: "@darbotlm/react-core/v2/context",
          external: true,
        };
      }
    }
...
export default defineConfig([
  {
    ...
    external: [
      ...
      "@darbotlm/core",
      "@darbotlm/shared",
      "@darbotlm/react-core/v2/context",
      "@darbotlm/web-inspector",
      "@darbotlm/a2ui-renderer",
      // Keep @darbotlm/web-components (the Lit drawer element) + its subpaths
  ...
  // file instead of being absorbed into shared chunks.
  {
    ...
    target: "es2022",
    outDir: "dist/v2",
    external: ["react", "@darbotlm/core", "@darbotlm/shared"],
  },
  // v2/headless: platform-agnostic hooks + CopilotKitCoreReact, used by
  // @darbotlm/react-native. All @darbotlm/* deps are external — they
  ...
  },
  {
    ...
    outputOptions(options) {
      ...
      options.globals = {
        ...
        "@darbotlm/core": "CopilotKitCore",
        "@darbotlm/shared": "CopilotKitShared",
        "@darbotlm/runtime-client-gql": "CopilotKitRuntimeClientGQL",
        "@darbotlm/web-inspector": "CopilotKitWebInspector",
        "@darbotlm/a2ui-renderer": "CopilotKitA2UIRenderer",
