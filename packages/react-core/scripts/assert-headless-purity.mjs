...
// stack. @darbotlm/react-native imports two react-core entries — /v2/headless
...
//     `@darbotlm/react-core/v2` entry (which links the whole render stack) left
...
//     through `@darbotlm/core`, which is external to this build: the entry says
//     only `from "@darbotlm/core"`, and grepping that string finds nothing.
...
if (isMain) {
  ...
  for (const file of targets) {
    ...
    if (!fs.existsSync(full)) {
      console.error(
        `ERROR: ${file} not found — run \`bun run build:sdk\` first`,
      );
      failed = true;
      continue;
    }
    ...
    if (opaque.length) {
      console.error(
        `ERROR: ${file} has edges this scan cannot follow, so it cannot be called clean:\n  ` +
          opaque.join("\n  "),
      );
      failed = true;
      continue;
    ...
    if (report.hits.length) {
      const detail = report.hits
        ...
        .join(", ");
      console.error(
        `ERROR: ${file} links the heavy render stack: ${detail}\n` +
    ...
    console.log(
      `OK: ${file} (${fs.statSync(full).size} B, ${report.inputCount} modules in graph) — clean`,
    );
  }
