# @darbotlm/channels-slack

The **Slack `PlatformAdapter`** for [`@darbotlm/channels`](../channels). It connects a
Slack workspace to any AG-UI agent: ingress via Bolt (Socket Mode), egress as
Block Kit rendered from the `@darbotlm/channels-ui` JSX vocabulary, plus text
streaming, opaque-id interactions, and HITL.

You write your UI as JSX once (`@darbotlm/channels-ui`) and drive the bot with
`@darbotlm/channels`; this package is the only one that talks to Slack.
...
```sh
pnpm add @darbotlm/channels-slack @darbotlm/channels @darbotlm/channels-ui
```

## Quickstart

```ts
import { createChannel } from "@darbotlm/channels";
import {
  slack,
  defaultSlackTools,
  defaultSlackContext,
} from "@darbotlm/channels-slack";
import { CopilotRuntime, CopilotKitIntelligence } from "@darbotlm/runtime/v2";
import { createCopilotNodeListener } from "@darbotlm/runtime/v2/node";
...
`@darbotlm/channels-ui` vocabulary to Block Kit: `Message → blocks`,
...
```tsx
import { Slack } from "@darbotlm/channels-slack";
...
```tsx
import { createChannel, defineChannelComponent } from "@darbotlm/channels";
import { Slack } from "@darbotlm/channels-slack";
import { z } from "zod";
...
Pass `feedback` to attach Slack's native AI feedback row (upvote/downvote,
...
`ChannelToolContext` from `@darbotlm/channels` (`{ thread, message?, user?, signal?,
...
[`@darbotlm/channels`](../channels/README.md):
