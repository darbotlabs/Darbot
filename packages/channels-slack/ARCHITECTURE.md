...
How `@darbotlm/channels-slack` is structured and **why** each boundary exists.

Application authors use this package with the product-facing
[`@darbotlm/channels`](../channels) umbrella. `SlackAdapter` imports and
implements [`PlatformAdapter`](../channels-core) from
[`@darbotlm/channels-core`](../channels-core). The channel engine owns the
...
[`@darbotlm/channels-core`](../channels-core). The members it implements:

```
SlackAdapter (`@darbotlm/channels-slack`)
...
- **No durable Slack-side state.** The next turn rebuilds context from Slack
  ...
  [`@darbotlm/channels` README](../channels/README.md), the product-facing
