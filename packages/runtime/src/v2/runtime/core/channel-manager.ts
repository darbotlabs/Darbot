...
// Type-only: @darbotlm/channels-core is pure-ESM, so a value import would break
...
// @darbotlm/channels shim that re-exports it: channels is a devDependency, so
 ...
 * `@darbotlm/channels-intelligence`'s `realtime-gateway.ts` rather than
...
/**
 * Minimal structural view of the `@darbotlm/channels-intelligence`
 ...
 */
const CHANNELS_INTELLIGENCE_SPECIFIER = "@darbotlm/channels-intelligence";

/**
 * Structural view of the `@darbotlm/channels-intelligence` module surface the
 ...
 * `@darbotlm/channels-intelligence` never becomes a static dependency of this
...
export async function defaultActivateChannel(
  ...
  try {
    ...
    if (isModuleNotFound(err)) {
      throw new Error(
        "Managed Channels require '@darbotlm/channels-intelligence' to be installed. Add it to your app's dependencies.",
        { cause: err },
      );
    }
    throw err;
