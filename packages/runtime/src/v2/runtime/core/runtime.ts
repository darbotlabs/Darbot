import type {
  ...
  NonEmptyRecord,
  RuntimeMode,
} from "@darbotlm/shared";
import { RUNTIME_MODE_SSE, RUNTIME_MODE_INTELLIGENCE } from "@darbotlm/shared";
import { createLicenseChecker } from "@darbotlm/license-verifier";
import type { LicenseChecker } from "@darbotlm/license-verifier";
import { resolveDebugConfig } from "@darbotlm/shared";
import type { ResolvedDebugConfig, DebugConfig } from "@darbotlm/shared";
...
// Type-only: @darbotlm/channels-core is the pure-ESM type source, so a value import would break this
// package's CJS output. The channels are validated + activated (wired to delivery
// transports) by `startChannels` from @darbotlm/channels-intelligence, called
// by the Channel-listener bootstrap — not here.
import type { Channel } from "@darbotlm/channels-core";
...
export interface OpenGenerativeUIOptions
  extends BaseCopilotRuntimeMiddlewareOptions {}
...
interface CopilotIntelligenceRuntimeBaseOptions
   ...
   * `@darbotlm/channels-intelligence` — not at construction.
   */
}
  ...
  implements CopilotIntelligenceRuntimeLike
{
  ...
  constructor(options: CopilotIntelligenceRuntimeOptions) {
    ...
    // `@darbotlm/channels-intelligence`, which this CJS package must not pull in.
