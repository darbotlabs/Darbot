...
third language: darbot knows a Bot only as an AG-UI endpoint URL, so a Python agent arrives exactly
...
to the model as external tools whose calls stream back to darbot to run through the governed gateway
