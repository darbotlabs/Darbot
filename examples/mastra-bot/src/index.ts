 ...
 *
 * This and the LangGraph example share no darbot-specific code beyond the AG-UI protocol. Tools
 * arrive in `input.tools` from the surface, so both drive a governed browser they have no direct
 * access to.
 *
 * `@ag-ui/mastra` exists and is the client side, it lets a darbotlm runtime call a Mastra agent. A
...
const bot = new Agent({
  name: "darbot Mastra coworker",
  instructions:
    "You are a Bot running on Mastra inside darbot. You have a real web browser available through " +
 ...
 * what puts every call through darbot's policy gateway instead of this process.
 */
