 ...
 *
 * This is a real LangGraph `StateGraph` with its own model client and tool loop. darbot knows it
 * only as an AG-UI endpoint URL.
 *
 * It is an AG-UI HTTP endpoint. One POST carrying a `RunAgentInput`, and
 * a stream of AG-UI events back. `@ag-ui/langgraph` is a client: it lets a darbotlm
...
function toLangChainMessages(input: RunAgentInput): BaseMessage[] {
  const messages: BaseMessage[] = [
    new SystemMessage(
      "You are a Bot running on LangGraph inside darbot. You have a real web browser available " +
