...
**AI coworkers you can hand real work to, and actually trust with the access.** Each gets a computer terminal of its own: a real browser with its own logins, its own files, and only the tool orchestrations you grant. Every action decided before it happens, logged and recorded when it happens, and introspective self reflection after.

[**darbot.ai/darbot**](https://darbot.ai/darbot) · [**Quick start**](#quick-start) · [**Features**](#features) · [**Bring your own agent**](#bring-your-own-agent) · [**Architecture**](#architecture) · [**Docs**](docs/README.md)

[![CI](https://github.com/darbotlm/darbot/actions/workflows/ci.yml/badge.svg)](https://github.com/darbotlm/darbot/actions/workflows/ci.yml)
[![security](https://github.com/darbotlm/darbot/actions/workflows/security_zizmor.yml/badge.svg)](https://github.com/darbotlm/darbot/actions/workflows/security_zizmor.yml)
...
2. Get darbotlm Intelligence credentials:
...
docker run -p 3001:3001 --env-file .env \
  -e EMBEDDED_POSTGRES=on -v darbot-data:/var/lib/postgresql \
  ghcr.io/darbotlm/darbot:latest

# Or the tree you have in front of you.
docker build -t darbot .
docker run -p 3001:3001 --env-file .env \
  -e EMBEDDED_POSTGRES=on -v darbot-data:/var/lib/postgresql darbot
```
...
- **Durable threads and memory**: conversations survive restarts through darbotlm Intelligence, and each deployment stamps the threads it owns.
...
`darbotlm_LICENSE_TOKEN` is optional. A self-hosted Intelligence with its own
...
| Variable                             | Use                                                                       |
| ------------------------------------ | ------------------------------------------------------------------------- |
| `darbot_SINGLE_USER`                | Admits every request as one administrator. Required when no identity provider is configured; `.env.example` ships it on. |
...
| `server`                 | 3001                       | Hono API, darbotlm runtime, auth, policy, audit, plugins, components, coworkers, and channels. |
...
| darbotlm Intelligence  | external                   | Durable threads and memory.                                                                      |
...
- `MICROSOFT_OAUTH_TENANT_ID` defaults to `common`, which admits personal Microsoft accounts as well
  ...
  darbot falls back to `upn` and then `preferred_username`. If none of the three arrives the
...
- [darbot.ai/darbot](https://darbot.ai/darbot)
